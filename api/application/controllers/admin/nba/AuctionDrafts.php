...
class AuctionDrafts extends API_Controller_Secure {
    ...
    public function add_post() {
        ...
        $this->form_validation->set_rules('ContestDuration', 'Contest Duration', 'trim');
       // $this->form_validation->set_rules('DailyDate', 'Day Date', 'trim' . (!empty($this->Post['ContestDuration']) && $this->Post['ContestDuration'] == 'Daily' ? '|required' : ''));
        ...
        $this->form_validation->set_rules('GameType', 'GameType', 'trim|required|in_list[Nfl,Ncaaf,Nba]');
        ...
        $TeamSize    = ($this->Post['GameType'] == "Nba") ? 32 : 130;
    ...
    function getSportsGameTypeConfiguration_post() {
        ...
        /* Get Contests Data */
        $SportsType = basketballGetConfiguration($this->Post['SubGameType']); 
        if (!empty($SportsType)) {
