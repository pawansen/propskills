app.controller('PageController', function ($scope, $http,$timeout){
    ...
    $scope.getList = function ()
    {
        ...
        $http.post(API_URL+'nba/sports/getTeams', data, contentType).then(function(response) {
            var response = response.data;
    ...
    $scope.loadFormEdit = function (Position, TeamGUID)
    {
        ...
        $scope.templateURLEdit = PATH_TEMPLATE+module+'/teams/edit_form.htm?'+Math.random();
    ...
    $scope.editData = function (Type,GUID,PowerFive)
    {
        ...
        $http.post(API_URL+'admin/nba/teams/editTeam', data, contentType).then(function(response) {
            var response = response.data;
            if(response.ResponseCode==200){ /* success case */               
