app.controller('PageController', function ($scope, $http,$timeout){
    ...
    $scope.getCountries = function ()
    {
        ...
        $http.post(API_URL+'state/getCountries', data, contentType).then(function(response) {
            var response = response.data;
            if(response.ResponseCode==200 && response.Data.Records){ /* success case */
                $scope.filterData = response.Data.Records;    
         }
        });
    }
    ...
    /*load delete form*/
    $scope.loadFormDelete = function (Position, StateName)
    {
        $scope.data.Position = Position;
        ...
        $http.post(API_URL+'state/getStateByName', 'SessionKey='+SessionKey+'&StateName='+StateName, contentType).then(function(response) {
            ...
            /*load delete form*/
    $scope.deleteDataState = function (StateName,CountryCode)
    {
        $http.post(API_URL+'state/deleteState', 'SessionKey='+SessionKey+'&StateName='+StateName+'&CountryCode='+CountryCode, contentType).then(function(response) {
            var response = response.data;
            if(response.ResponseCode==200){ /* success case */
    ...
    $scope.addData = function ()
    {
        ...
        $http.post(API_URL+'state/add', data, contentType).then(function(response) {
            var response = response.data;
            if(response.ResponseCode==200){ /* success case */               
                alertify.success(response.Message);
                $('.modal-header .close').click();
