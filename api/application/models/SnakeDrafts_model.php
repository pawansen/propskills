...
class SnakeDrafts_model extends CI_Model {
    ...
    function addAuctionPlayer($SeriesID, $ContestID, $WeekID, $ContestDuration, $DailyDate) {
        ...
        // $Query = $this->db->query("SELECT TeamIDLocal,TeamIDVisitor FROM sports_teams WHERE SeriesID = '" . $SeriesID . "' AND DATE(MatchStartDateTime) = '" .$DailyDate . "'");
        ...
        if ($Query->num_rows() > 0) {
            if ($multiRecords) {
                ...
                foreach ($Query->result_array() as $key => $Record) {
                    ...
                    $Records[$key]['Scoring'] = (!empty($Record['Scoring'])) ? json_decode($Record['Scoring']) : new stdClass();
                    $Records[$key]['Defense'] = (!empty($Record['Defense'])) ? json_decode($Record['Defense']) : new stdClass();
                    $Records[$key]['Passing'] = (!empty($Record['Passing'])) ? json_decode($Record['Passing']) : new stdClass();
                    $Records[$key]['Receiving'] = (!empty($Record['Receiving'])) ? json_decode($Record['Receiving']) : new stdClass();
                    $Records[$key]['Rushing'] = (!empty($Record['Rushing'])) ? json_decode($Record['Rushing']) : new stdClass();
                ...
                $Record['Scoring'] = (!empty($Record['Scoring'])) ? json_decode($Record['Scoring']) : new stdClass();
                $Record['Defense'] = (!empty($Record['Defense'])) ? json_decode($Record['Defense']) : new stdClass();
                $Record['Passing'] = (!empty($Record['Passing'])) ? json_decode($Record['Passing']) : new stdClass();
                $Record['Receiving'] = (!empty($Record['Receiving'])) ? json_decode($Record['Receiving']) : new stdClass();
                $Record['Rushing'] = (!empty($Record['Rushing'])) ? json_decode($Record['Rushing']) : new stdClass();
        ...
        if (!empty($Field)) {
            ...
            $FieldArray = array(
                ...
                'IsAutoDraft' => '(SELECT IF( EXISTS(
                                ...
                                WHERE sports_contest_join.ContestID =  C.ContestID AND UserID = ' . @$Where['SessionUserID'] . ' AND  sports_contest_join.IsAutoDraft="Yes" LIMIT 1), "Yes", "No")) AS IsAutoDraft',
                'PlayerIDLive' => 'P.PlayerIDLive',
                'PlayerPic' => 'IF(P.PlayerPic IS NULL,CONCAT("' . BASE_URL . '","uploads/PlayerPic/","player.png"),CONCAT("' . BASE_URL . '","uploads/PlayerPic/",P.PlayerPic)) AS PlayerPic',
                'PlayerCountry' => 'P.PlayerCountry',
                'PlayerBattingStyle' => 'P.PlayerBattingStyle',
        ...
        if ($Query->num_rows() > 0) {
            if ($multiRecords) {
                ...
                foreach ($Query->result_array() as $key => $Record) {
                    ...
                    if(!empty($Where['WireHistoryStatus']) && $Where['WireHistoryStatus'] == "Yes"){
    ...
    function getPlayersAll($Field = '', $Where = array(), $multiRecords = FALSE, $PageNo = 1, $PageSize = 15) {
        ...
        $this->db->select('P.PlayerGUID,P.PlayerName,P.YardsPerGame,P.Yards,P.TotalTouchdowns,P.WeeklyStats');
        if (!empty($Field))
        $this->db->select($Field, FALSE);
        ...
        $this->db->where("P.Position !=", 'Defense');
        /* Total records count only if want to get multiple records */
        if ($multiRecords) {
            $TempOBJ = clone $this->db;
            $TempQ = $TempOBJ->get();
        ...
        if ($Query->num_rows() > 0) {
            if ($multiRecords) {
                ...
                $Record = $Query->row_array();
                        ...
                        $WeekStats['fumbles_lost'] = 0;
                        $WeekStats['total_tackles'] = 0;
                $Record['PlayerBattingStats'] = (!empty($Record['WeeklyStats'])) ? json_decode($Record['WeeklyStats']) : new stdClass();
                $Record['PointsData'] = (!empty($Record['PointsData'])) ? json_decode($Record['PointsData'], TRUE) : $WeekStats;
                return $Record;
            }
        }
        return FALSE;
    ...
    function getPlayersMyTeam($Field = '', $Where = array(), $multiRecords = FALSE, $PageNo = 1, $PageSize = 15) {
        ...
        $this->db->select('P.PlayerGUID,P.PlayerName,C.ContestSize,C.RosterSize,UTP.PlayerSelectTypeRole,C.Privacy,P.Passing,P.Rushing,P.Receiving,P.Scoring,P.Defense,P.WeeklyStats');
        if (!empty($Field))
        $this->db->select($Field, FALSE);
        $this->db->from('tbl_entity E, sports_players P,sports_series S,sports_contest C,sports_teams T,sports_users_teams UT, sports_users_team_players UTP');
        $this->db->where("P.PlayerID", "E.EntityID", FALSE);
        $this->db->where("UTP.PlayerID", "P.PlayerID", FALSE);
        $this->db->where("UT.UserTeamID", "UTP.UserTeamID", FALSE);
        ...
        if (!empty($Where['OrderBy']) && !empty($Where['Sequence'])) {
            $this->db->order_by($Where['OrderBy'], $Where['Sequence'],FALSE);
        }else if (!empty($Where['RandData'])) {
            $this->db->order_by($Where['RandData']);
        } else {
            $this->db->order_by('P.PlayerSalary', 'DESC');
            //$this->db->order_by('P.PlayerID', 'DESC');
            ...
            if ($multiRecords) {
                ...
                foreach ($Query->result_array() as $key => $Record) {
                    ...
                    if(!empty($Record['Passing'])){
                        $Records[$key]['Passing'] = $Passing = json_decode($Record['Passing'],true);
                        $WeekStats['yards'] = $Passing['yards'];
                        $WeekStats['interceptions'] = $Passing['interceptions'];
                        $WeekStats['passing_touchdowns'] = $Passing['passing_touchdowns'];
                    }
                    if(!empty($Record['Rushing'])){
                        $Records[$key]['Rushing'] = $Rushing = json_decode($Record['Rushing'],true);
                        $WeekStats['fumbles'] += $Rushing['fumbles'];
                        $WeekStats['rushing_yards'] = $Rushing['rushing_yards'];
                        $WeekStats['yards_per_game'] = $Rushing['yards_per_game'];
                        $WeekStats['rushing_touchdowns'] = $Rushing['rushing_touchdowns'];
                        $WeekStats['fumbles_lost'] += $Rushing['fumbles_lost'];
                    }
                    if(!empty($Record['Receiving'])){
                    ...
                    if(!empty($Record['WeeklyStats'])){
                        $Records[$key]['WeeklyStats'] = $WeeklyStats = json_decode($Record['WeeklyStats'],true);
                        $WeekStats['total_points'] = $WeeklyStats['total_points'];
                    }
                    ...
                    if($Records['Privacy'] == 'No'){
                        $Records[$key]['RoosterRole'] = footballGetConfigurationPlayersRooster($Record['ContestSize']);
                    }else{
                        $Records[$key]['RoosterRole'] = footballGetConfigurationPlayersRoosterPrivate($Record['RosterSize']);
                    }
                }
                if (!empty($Where['MySquadPlayer']) && $Where['MySquadPlayer'] == "Yes") {
                    $Return['Data']['IsAssistant'] = $IsAssistant;
    ...
    function getUserInLive() {
        ...
        $Return["Status"] = 0;
        /** check draft in live * */
        $DraftGames = $this->getContests('ContestID,AuctionStatus,SeriesID,SeriesGUID,GameType', array('AuctionStatusID' => 2, 'LeagueType' => "Draft"), TRUE, 1);
        if ($DraftGames['Data']['TotalRecords'] > 0) {
            $Users = array();
            foreach ($DraftGames['Data']['Records'] as $Key => $Draft) {
    ...
    function addDraftUserTeam($UserID, $ContestID, $SeriesID, $WeekStart) {
    ...
    function addDraftUserTeamSquadAssistant($UserTeamID, $UserID, $ContestID, $SeriesID, $Team, $RoosterSize) {
        $Return = array();
        $Return["Status"] = 0;
        $Return["Player"] = array();
        /** dynamic player role * */
        $ContestCriteria = $this->getContests('ContestID,DraftPlayerSelectionCriteria', array("ContestID" => $ContestID), FALSE, 1);

        $DraftPlayerSelectionCriteria = (!empty($ContestCriteria['DraftPlayerSelectionCriteria'])) ? $ContestCriteria['DraftPlayerSelectionCriteria'] : array('QB' => 1, "RB" => 2, "WR" => 3, "TE" => 1,"DEF"=>1);
        ...
        if ($Rows > 0) {
            // $DraftSquad = $Query->result_array();
            // foreach ($DraftSquad as $Key => $PlayerSquad) {
            //     $DraftSquad[$Key]['PlayerRole'] = $this->db->query('SELECT S.PlayerRole from `tbl_auction_player_bid_status` S  WHERE S.ContestID = "' . $ContestID . '" AND S.PlayerID = "' . $PlayerSquad['PlayerID'] . '"')->row()->PlayerRole;
            // }
            /** check is assistant and unsold player * */
            $this->db->select("UTP.PlayerID,UT.UserTeamID,UT.UserID,BS.PlayerStatus");
            $this->db->from('sports_users_teams UT, sports_users_team_players UTP,tbl_auction_player_bid_status BS,sports_players ST');
            $this->db->where("UT.UserTeamID", "UTP.UserTeamID", FALSE);
    ...
    function addDraftUserTeamSquadIsPlayer($UserTeamID, $UserID, $ContestID, $SeriesID, $Team, $RoosterSize, $PlayerRole='') {
        ...
        $DraftPlayerSelectionCriteria = (!empty($ContestCriteria['DraftPlayerSelectionCriteria'])) ? $ContestCriteria['DraftPlayerSelectionCriteria'] : array('QB' => 1, "RB" => 2, "WR" => 3, "TE" => 1,"FLEX"=>1);
        /** dynamic player role * */
        $DraftTeamPlayerLimit = $RoosterSize;
        /** check is assistant and unsold player * */
        ...
        if ($Rows > 0) {
                $DraftSquad = $Query->result_array();
                /*foreach ($DraftSquad as $Key => $PlayerSquad) {
                    $DraftSquad[$Key]['PlayerRole'] = $this->db->query('SELECT (CASE S.PlayerRole
                             when "QuarterBack" then "QB"
                             when "RunningBack" then "RB"
                             when "WideReceiver" then "WR"
                             when "TightEnd" then "TE"
                             ...
                             when "DefenseTackle" then "DEF"
                             when "Safety" then "DEF"
                             END) as PlayerRole from `tbl_auction_player_bid_status` S  WHERE S.ContestID = "' . $ContestID . '" AND S.PlayerID = "' . $PlayerSquad['PlayerID'] . '"')->row()->PlayerRole;
                }*/
            /** check player role condition * */
            $PlayerRoles = array_count_values(array_column($DraftSquad, 'PlayerRole'));
            ...
            if ($DraftLiveRound <= $CriteriaRounds) {
                ...
                if ($Team['PlayerRole'] == "QB" && $PlayerRole !='FLEX' && $PlayerRole !='WR/TE') {
                    if (@$PlayerRoles['QB'] < $DraftPlayerSelectionCriteria['QB']) {
                        /** check total player in squad * */
                        if ($Rows < $DraftTeamPlayerLimit) {
                            /** insert user team player squad * */
                            $InsertData = array(
                        ...
                        $Return["Status"] = 0;
                        $Return['Message'] = "Minimum Criteria for QB is fulfilled. Please select player for another position will you complete the minimum criteria of '".$RoosterSize."' Players";
                        return $Return;
                    }
                }

                if ($Team['PlayerRole'] == "RB" && $PlayerRole !='FLEX' && $PlayerRole !='WR/TE') {
                    if (@$PlayerRoles['RB'] < $DraftPlayerSelectionCriteria['RB']) {
                        /** check total player in squad * */
                        if ($Rows < $DraftTeamPlayerLimit) {
                            /** insert user team player squad * */
                            $InsertData = array(
                        ...
                        $Return["Status"] = 0;
                        $Return['Message'] = "Minimum Criteria for RB is fulfilled. Please select player for another position will you complete the minimum criteria of '".$RoosterSize."' Players";
                        return $Return;
                    }
                }

                if ($Team['PlayerRole'] == "WR" && $PlayerRole !='FLEX' && $PlayerRole !='WR/TE') {
                    if (@$PlayerRoles['WR'] < $DraftPlayerSelectionCriteria['WR']) {
                        /** check total player in squad * */
                        if ($Rows < $DraftTeamPlayerLimit) {
                            /** insert user team player squad * */
                            $InsertData = array(
                        ...
                        $Return["Status"] = 0;
                        $Return['Message'] = "Minimum Criteria for WR is fulfilled. Please select player for another position will you complete the minimum criteria of '".$RoosterSize."' Players";
                        return $Return;
                    }
                }

                if ($Team['PlayerRole'] == "TE" && $PlayerRole !='FLEX' && $PlayerRole !='WR/TE') {
                    if (@$PlayerRoles['TE'] < $DraftPlayerSelectionCriteria['TE']) {
                        /** check total player in squad * */
                        if ($Rows < $DraftTeamPlayerLimit) {
                            /** insert user team player squad * */
                            $InsertData = array(
                        ...
                        $Return["Status"] = 0;
                        $Return['Message'] = "Minimum Criteria for TE is fulfilled. Please select player for another position will you complete the minimum criteria of '".$RoosterSize."' Players";
                        return $Return;
                    }
                }
                if ($PlayerRole == "FLEX") {
                    $FLEX = $this->db->query('SELECT PlayerID from `sports_users_team_players` WHERE 
                            UserTeamID = "' . $UserTeamID . '" AND PlayerSelectTypeRole = "FLEX" ')->row()->PlayerID;
                    if (empty($FLEX)) {
                        /** check total player in squad * */
                        if ($Rows < $DraftTeamPlayerLimit) {
                            /** insert user team player squad * */
                ...
                if ($PlayerRole == "WR/TE") {
                    $FLEX = $this->db->query('SELECT PlayerID from `sports_users_team_players` WHERE 
                            UserTeamID = "' . $UserTeamID . '" AND PlayerSelectTypeRole = "WR/TE" ')->row()->PlayerID;
                    if (empty($FLEX)) {
                        /** check total player in squad * */
                        if ($Rows < $DraftTeamPlayerLimit) {
                            /** insert user team player squad * */
                        ...
                        $Return["Status"] = 0;
                        $Return['Message'] = "Minimum Criteria for WR/TE is fulfilled. Please select player for another position will you complete the minimum criteria of '".$RoosterSize."' Players";
                        return $Return;
                    }
                }
    ...
    function addDraftUserTeamSquadNotAssistant($UserTeamID, $UserID, $ContestID, $SeriesID, $Team, $RoosterSize) {
            ...
            if ($DraftLiveRound <= $CriteriaRounds) {
                    ...
                    if (@$PlayerRoles['QB'] < $DraftPlayerSelectionCriteria['QB']) {
                        ...
                        if ($Rows < $DraftTeamPlayerLimit) {
                            ...
                            $this->db->where("BS.PlayerID", "ST.PlayerID", FALSE);
                            $this->db->where_in("BS.PlayerStatus", array("Live", "Upcoming"));
                            $this->db->where("BS.PlayerRole", "QuarterBack");
                            $this->db->where("BS.ContestID", $ContestID);
                            $this->db->where("BS.SeriesID", $SeriesID);
                    ...
                    if (@$PlayerRoles['RB'] < $DraftPlayerSelectionCriteria['RB']) {
                        ...
                        if ($Rows < $DraftTeamPlayerLimit) {
                            ...
                            $this->db->where("BS.PlayerID", "ST.PlayerID", FALSE);
                            $this->db->where_in("BS.PlayerStatus", array("Live", "Upcoming"));
                            $this->db->where("BS.PlayerRole", "RunningBack");
                            $this->db->where("BS.ContestID", $ContestID);
                            $this->db->where("BS.SeriesID", $SeriesID);
                            $this->db->order_by("ST.YardsPerGame", "DESC");
                            $this->db->limit(1);
                    ...
                    if (@$PlayerRoles['TE'] < $DraftPlayerSelectionCriteria['TE']) {
                        ...
                        if ($Rows < $DraftTeamPlayerLimit) {
                            ...
                            $this->db->where("BS.PlayerID", "ST.PlayerID", FALSE);
                            $this->db->where_in("BS.PlayerStatus", array("Live", "Upcoming"));
                            $this->db->where("BS.PlayerRole", "TightEnd");
                            $this->db->where("BS.ContestID", $ContestID);
                            $this->db->where("BS.SeriesID", $SeriesID);
                            $this->db->order_by("ST.YardsPerGame", "DESC");
                            $this->db->limit(1);
    ...
    function draftPlayerSold($Input = array(), $SeriesID, $ContestID, $UserID, $PlayerID = "") {
        ...
        $DraftGames = $this->getContests('ContestID,SeriesID,SeriesGUID,DraftTotalRounds,TotalJoined,DraftLiveRound,RosterSize,WeekStart,Privacy', array('LeagueType' => "Draft", "ContestID" => $ContestID, "SeriesID" => $SeriesID), TRUE, 1);
        $RoosterSize = ($DraftGames['Data']['TotalRecords'] > 0) ? $DraftGames['Data']['Records'][0]['RosterSize'] : 0;
        $WeekStart = ($DraftGames['Data']['TotalRecords'] > 0) ? $DraftGames['Data']['Records'][0]['WeekStart'] : 0;
        ...
        if ($Query->num_rows() > 0) {
            ...
            if (empty($PlayerID)) {
                ...
                if ($diffSeconds >= $UserTimer) {
                    /** check is assistant and unsold player * */
                    $this->db->select("UTP.PlayerID,UT.UserTeamID,UT.UserID,BS.PlayerStatus,ST.PlayerName");
                    $this->db->from('sports_users_teams UT, sports_users_team_players UTP,tbl_auction_player_bid_status BS,sports_players ST');
                    $this->db->where("UT.UserTeamID", "UTP.UserTeamID", FALSE);
                    $this->db->where("UT.ContestID", "BS.ContestID", FALSE);
                    $this->db->where("UT.SeriesID", "BS.SeriesID", FALSE);
                    ...
                    if ($Query->num_rows() > 0) {
                        ...
                        foreach ($AssistantPlayers as $Player) {
                            ...
                            $UserTeamID = $this->addDraftUserTeam($UserID, $ContestID, $SeriesID, $WeekStart);
                            if ($UserTeamID) {
                                $Status = $this->addDraftUserTeamSquadAssistant($UserTeamID, $UserID, $ContestID, $SeriesID, $Player, $RoosterSize);
                                if (empty($Status)) {
                                    /** check is assistant and unsold player * */
                                    ...
                                    if ($Query->num_rows() > 0) {
                                        ...
                                        foreach ($AllPlayer as $Player) {
                                            ...
                                            $UserTeamID = $this->addDraftUserTeam($UserID, $ContestID, $SeriesID, $WeekStart);
                                            if ($UserTeamID) {
                                                $Status = $this->addDraftUserTeamSquadNotAssistant($UserTeamID, $UserID, $ContestID, $SeriesID, $Player, $RoosterSize);
                                                if ($Status['Status'] == 1) {
                ...
                if ($Query->num_rows() > 0) {
                    ...
                    foreach ($AllPlayer as $Player) {
                        ...
                        $UserTeamID = $this->addDraftUserTeam($UserID, $ContestID, $SeriesID, $WeekStart);
                        if ($UserTeamID) {
                            $Status = $this->addDraftUserTeamSquadIsPlayer($UserTeamID, $UserID, $ContestID, $SeriesID, $Player, $RoosterSize, $Input['PlayerRole']);
                            if ($Status['Status'] == 1) {
                                $Return["Data"]["Status"] = 1;
                                $Return['Message'] = "Successfully player added";
                            } else {
    ...
    function getContests($Field = '', $Where = array(), $multiRecords = FALSE, $PageNo = 1, $PageSize = 15) {
        ...
        if (!empty($Field)) {
            ...
            $FieldArray = array(
                ...
                'SubGameType' => 'CASE C.SubGameType
                             when "ProFootballRegularSeasonOwners" then "NFL (Regular Season)"
                             when "ProFootballPlayoffs" then "NFL (Playoffs)"
                             when "ProFootballPreSeasonOwners" then "NFL (Pre Season)"
                             ...
                             END as SubGameType',
                'GamePlayTypeDescription' => 'CASE C.GamePlayType
                             when "PICK_5" then "Pick 5 Fantasy"
                             when "CLASSIC_9" then "Classic 9 Fantasy"
                             when "PROP_5" then "Prop 5 Fantasy"
                             when "TD_ONLY" then "TD Only"
                             END as GamePlayTypeDescription',
                'InvitePermission'=>'C.InvitePermission',
                'PrivatePointSystem' => 'IF(C.Privacy = "Yes", C.PrivatePointScoring, "") AS PrivatePointSystem',
                'isWeekStarted' => '(SELECT IF((SELECT E.StatusID FROM sports_matches M JOIN tbl_entity E WHERE M.MatchID = E.EntityID AND M.WeekID = C.WeekStart ORDER BY MatchStartDateTime ASC LIMIT 1) = "1", "No", "Yes")) AS isWeekStarted',
        ...
        if (!empty($Where['Keyword'])) {
            if (is_array(json_decode($Where['Keyword'], true))) {
                ...
                if (isset($Where['Keyword']['ContestStartDateDay'])) {

                    $this->db->where("DATE(C.DailyDate)", @$Where['Keyword']['ContestStartDateDay']);
                }
                if (isset($Where['Keyword']['MatchStartDate'])) {

                    $this->db->where("DATE(C.DailyDate)", @$Where['Keyword']['MatchStartDate']);
                }
        ...
        if (!empty($Where['IsAutoCreate'])) {
            $this->db->where("C.IsAutoCreate", $Where['IsAutoCreate']);
        }
        ...
        if ($Query->num_rows() > 0) {
            if ($multiRecords) {
                ...
                foreach ($Query->result_array() as $key => $Record) {
                    ...
                    /** get week info * */
                    if (isset($Where['WeekInfo']) && $Where['WeekInfo'] == "Yes") {
                        $Records[$key]['WeekTeamInfo'] = array();
                        /** get Complete Week List * */
                        ...
                        $Query = $this->db->get();
                        if ($Query->num_rows() > 0) {
                            $Records[$key]['WeekTeamInfo'] = $Query->result_array();
                        }
                    }
    ...
    function joinContest($Input = array(), $SessionUserID, $ContestID, $SeriesID, $UserTeamID, $IsAutoDraft) {
    ...
    function autoDraftOnOff($Input = array(), $SessionUserID, $SeriesID, $ContestID, $UserTeamID) {
        ...
        $this->db->where('ContestID', $ContestID);
        $this->db->limit(1);
        $this->db->update('sports_contest_join', array('IsAutoDraft' => @$Input['IsAutoDraft']));
    ...
    function getJoinedDraftAllTeams($ContestID, $SeriesID, $UserID) {
        $ContestRound = $this->SnakeDrafts_model->getContests('ContestID,DraftLiveRound,DraftTotalRounds,DraftTotalRounds', array("LeagueType" => "Draft", "ContestID" => $ContestID, "SeriesID" => $SeriesID));
        $DraftTotalRounds = $ContestRound['DraftLiveRound'];
       // $DraftTotalRounds = $ContestRound['DraftTotalRounds'];
        $AllTeams = array();
        $Rounds = array();
                ...
                foreach ($AllJoinedUsers['Data']['Records'] as $Rows) {
                    $this->db->select('UT.UserID,UT.UserTeamID,UTP.PlayerID,UTP.DateTime,T.PlayerGUID,
                        ...
                        (CASE T.PlayerRole
                             ...
                             when "Safety" then "DEF"
                             END ) as PlayerRoleShort');
                    $this->db->from('sports_users_teams UT, sports_users_team_players UTP, sports_players T');
                    $this->db->where("UT.UserTeamID", "UTP.UserTeamID", FALSE);
                    ...
                    if ($Query->num_rows() > 0) {
                        ...
                        $PlayersAssistant['PlayerDateTime'] =  date('Y-m-d H:i:s', strtotime($date. ' + 2 days'));
                        $Temp[] = $PlayersAssistant;
                    }
                }
                if ($i % 2 == 0) {
    ...
    function getJoinedDraftAllTeamsHistory($ContestID, $SeriesID, $UserID) {
        $ContestRound = $this->SnakeDrafts_model->getContests('ContestID,DraftLiveRound,DraftTotalRounds,DraftTotalRounds', array("LeagueType" => "Draft", "ContestID" => $ContestID, "SeriesID" => $SeriesID));
        $DraftTotalRounds = $ContestRound['DraftLiveRound'];
       // $DraftTotalRounds = $ContestRound['DraftTotalRounds'];
        $AllTeams = array();
    ...
    function getJoinedDraftAllTeamsOLD($ContestID, $SeriesID, $UserID) {
        $ContestRound = $this->SnakeDrafts_model->getContests('ContestID,DraftLiveRound,DraftTotalRounds,DraftTotalRounds', array("LeagueType" => "Draft", "ContestID" => $ContestID, "SeriesID" => $SeriesID));
        //$DraftTotalRounds = $ContestRound['DraftLiveRound'];
        $DraftTotalRounds = $ContestRound['DraftTotalRounds'];
        $AllTeams = array();
...
//            }
    ...
    function editUserTeam($Input = array(), $UserTeamID) {
        ...
        if (!empty($Input['UserTeamPlayers'])) {
            ...
            if (empty($Input['SeriesID'])) {
                $Input['SeriesID'] = $this->db->query('SELECT SeriesID FROM sports_users_teams WHERE UserTeamID = ' . $UserTeamID . ' LIMIT 1')->row()->SeriesID;
            }
    ...
    function getJoinedContestsUsers($Field = '', $Where = array(), $multiRecords = FALSE, $PageNo = 1, $PageSize = 15) {
        ...
        if (!empty($Field)) {
            ...
            $FieldArray = array(
                ...
                'WeekTotalPoints' => '(SELECT TotalPoints FROM sports_users_teams WHERE sports_users_teams.UserID=JC.UserID AND sports_users_teams.ContestID=JC.ContestID AND IsPreTeam="No") WeekTotalPoints',
            );
            if ($Params) {
                foreach ($Params as $Param) {
                    $Field .= (!empty($FieldArray[$Param]) ? ',' . $FieldArray[$Param] : '');
        ...
        if ($Query->num_rows() > 0) {
            if ($multiRecords) {
                ...
                foreach ($Return['Data']['Records'] as $key => $record) {
                    ...
                    $Return['Data']['Records'][$key]['TotalPoints'] = $record['WeekTotalPoints'];
                    /*if (!empty($Where['WeekID'])) {
                        ...
                        if ($Query->num_rows() > 0) {
                            ...
                            $Return['Data']['Records'][$key]['TotalPoints'] = $Points['TotalPoints'];
                        }
                    }*/
                }
                return $Return;
    ...
    function contestUserLeaderboard($Field = '', $Where = array(), $multiRecords = FALSE, $PageNo = 1, $PageSize = 15) {
        ...
        if (!empty($Field)) {
            ...
            $Field = '';
            $FieldArray = array(
                 'TotalPointsSeason' => '(SELECT TotalPoints FROM sports_contest_join WHERE sports_contest_join.UserID=UTW.UserID AND sports_contest_join.ContestID=UTW.ContestID) TotalPointsSeason',
    ...
    function leaveDraftRoom($ContestID, $UserID) {
        ...
        foreach ($ContestsUsers['Data']['Records'] as $Value) {
            ...
            foreach ($JoinedContestsUsers['Data']['Records'] as $Rows) {
                ...
                /* Delete Team Players */
                $this->db->delete('sports_contest_join', array('UserID' => $UserID, 'ContestID' => $ContestID));
    ...
    function draftTeamPlayersSubmit($Input = array(), $UserTeamID, $SeriesID, $SessionUserID, $GameType) {
    ...
    function getMySquadTeam($ContestID, $WeekID, $SessionUserID, $SubGameTypeKey, $StatusID = "") {
        $AllMatchesList = array();
    ...
    function getDraftPlayerDropAddTransactions($ContestID) {
                ...
                $this->db->select("U.UserTeamCode,P.PlayerName,P.PlayerGUID,"
                        ...
                        . '(CASE P.PlayerRole
                             ...
                             when "Safety" then "DEF"
                             END) as PlayerRoleShort');
                $this->db->from('sports_wire_history H,sports_players P,tbl_users U');
    ...
    function getWeekDate($SeriesID, $WeekID) {
        ...
        $Invite = array();
        /** get upcoming week * */
        $this->db->select("DATE_FORMAT(M.MatchStartDateTime,'%Y-%m-%d') MatchStartDateTime");
        ...
        $this->db->order_by("M.MatchStartDateTime", "ASC");
        $this->db->group_by('date(M.MatchStartDateTime)');
        // $this->db->limit(1);
        $Query = $this->db->get();
        if ($Query->num_rows() > 0) {
            $Invite = $Query->result_array();
    ...
    function getWeekDateOld($SeriesID, $WeekID) {
    ...
    function getCurrentWeekLast($SeriesID='') {
    ...
    function removeDraftPlayer($ContestID, $SessionUserID, $UserTeamID, $PlayerID, $PlayerSelectTypeRole) {
            ...
            $this->db->from('sports_users_teams U,sports_users_team_players UTP');
            ...
            /** check catch team id availabe or not * */
            $this->db->select("ContestID,ContestDuration,DailyDate,WeekStart");
            $this->db->from('sports_contest');
            $this->db->where("ContestID", $ContestID);
            $Query = $this->db->get();
            if ($Query->num_rows() > 0) {
               $Contest = $Query->row_array();
                ...
                if($Contest['ContestDuration'] == 'Daily'){
                    $Query = $this->db->query("SELECT `M`.`MatchID`
                                                ...
                                                WHERE M.WeekID='".$Contest['WeekStart']."' AND DATE(MatchStartDateTime) ='" . $Contest['DailyDate'] . "'
                                                 LIMIT 1");
                    if($Query->num_rows() > 0){
                        $M = $Query->row_array();
                    ...
                    $Query = $this->db->query("SELECT `M`.`MatchID`
                                                FROM `sports_matches` `M`
                                                WHERE M.WeekID='".$Contest['WeekStart']."' AND DATE(MatchStartDateTime) <='" . date('Y-m-d') . "'
                                                 LIMIT 1");
                    if($Query->num_rows() > 0){
                        $M = $Query->row_array();
                        $this->db->select("EntityID");
               ...
               /*if($Contest['ContestDuration'] == 'Daily'){
                    $Query = $this->db->query("SELECT `M`.`MatchID`
                                                FROM `sports_matches` `M`,sports_team_players UTP
                                                WHERE  UTP.MatchID=M.MatchID AND UTP.PlayerID='".$PlayerID."' AND M.WeekID='".$Contest['WeekStart']."' AND DATE(MatchStartDateTime) ='" . $Contest['DailyDate'] . "'
                                                 LIMIT 1");
                    if($Query->num_rows() > 0 && $Contest['DailyDate'] == date('Y-m-d')){
                        $M = $Query->row_array();
                        $this->db->select("EntityID");
                        $this->db->from('tbl_entity');
                        $this->db->where("EntityID", $M['MatchID']);
                    ...
                    $Query = $this->db->query("SELECT `M`.`MatchID`
                                                FROM `sports_matches` `M`,sports_team_players UTP
                                                WHERE  UTP.MatchID=M.MatchID AND UTP.PlayerID='".$PlayerID."' AND M.WeekID='".$Contest['WeekStart']."' AND DATE(MatchStartDateTime) <='" . date('Y-m-d') . "'
                                                 LIMIT 1");
                    if($Query->num_rows() > 0){
                        $M = $Query->row_array();
                        $this->db->select("EntityID");
    ...
    function addDraftPlayer($ContestID, $SessionUserID, $UserTeamID, $PlayerID, $PlayerSelectTypeRole) {
            ...
            $Return['Message'] = "Error";
                        ...
                        $Query = $this->db->get();
                        if ($Query->num_rows() > 0) {
                            $Return['Message'] = "This player has already been on a roster";
                            return $Return;
                        }
            ...
            /** check catch team id availabe or not * */
            $this->db->select("ContestID,ContestDuration,DailyDate,WeekStart,SeriesID,RosterSize,DraftPlayerSelectionCriteria");
            $this->db->from('sports_contest');
            $this->db->where("ContestID", $ContestID);
            $Query = $this->db->get();
            if ($Query->num_rows() > 0) {
                ...
                if ($Rows > 0) {
                     ...
                     if ($PlayerSelectTypeRole == "QB") {
                        if (@$PlayerRoles['QB'] >= $DraftPlayerSelectionCriteria['QB']) {
                            $Return['Message'] = "Minimum Criteria for QB is fulfilled. Please select player for another position will you complete the minimum criteria of '".$RoosterSize."' Roosters";
                            return $Return;
                        }
                     }

                    if ($PlayerSelectTypeRole == "RB") {
                        if (@$PlayerRoles['RB'] >= $DraftPlayerSelectionCriteria['RB']) {
                            $Return['Message'] = "Minimum Criteria for RB is fulfilled. Please select player for another position will you complete the minimum criteria of '".$RoosterSize."' Roosters";
                            return $Return;
                        }
                     }

                    if ($PlayerSelectTypeRole == "WR") {
                        if (isset($DraftPlayerSelectionCriteria['WR/TE']) && empty($PlayerRoles['WR/TE'])) {
                            $DraftPlayerSelectionCriteria['WR'] += 1;
                            $PlayerSelectTypeRole = "WR/TE";
                        }
                        if (@$PlayerRoles['WR'] >= $DraftPlayerSelectionCriteria['WR']) {
                            $Return['Message'] = "Minimum Criteria for WR is fulfilled. Please select player for another position will you complete the minimum criteria of '".$RoosterSize."' Roosters";
                            return $Return;
                        }
                     }

                    if ($PlayerSelectTypeRole == "TE") {
                        if (isset($DraftPlayerSelectionCriteria['WR/TE']) && (empty($PlayerRoles['TE']) && $PlayerRoles['WR'] == 1)) {
                            $DraftPlayerSelectionCriteria['TE'] += 1;
                            $PlayerSelectTypeRole = "WR/TE";
                        }
                        if (@$PlayerRoles['TE'] >= $DraftPlayerSelectionCriteria['TE']) {
                            $Return['Message'] = "Minimum Criteria for TE is fulfilled. Please select player for another position will you complete the minimum criteria of '".$RoosterSize."' Roosters";
                            return $Return;
                        }
                     }
                    ...
                    if ($PlayerSelectTypeRole == "WR/TE") {
                        if (@$PlayerRoles['WR/TE'] >= $DraftPlayerSelectionCriteria['WR/TE']) {
                            $Return['Message'] = "Minimum Criteria for WR/TE is fulfilled. Please select player for another position will you complete the minimum criteria of '".$RoosterSize."' Roosters";
                            return $Return;
                        }
                     }
               ...
               /*if($Contest['ContestDuration'] == 'Daily'){
                ...
                $Insert = $this->db->insert('sports_users_team_players', $WireHistory);
    ...
    function addUserTeamDraft($SessionUserID, $SeriesID, $ContestID, $StatusID = 2) {
        ...
        $Query = $this->db->query('SELECT WeekStart FROM sports_contest WHERE ContestID = "' . $ContestID . '" LIMIT 1');
        $WeekStart = $Query->row()->WeekStart;
    ...
    */
    function autoCreateSeasonLongWeeklyDraft() {
      ...
      Description: auto Create SeasonLong Weekly Contest
    */
    function autoCompleteWeeklyDaily() {

        /*Get Running Contest */ 
        $this->db->select("SeriesID,ContestID,WeekStart as WeekID,DailyDate,ContestDuration,WeekEnd");
        $this->db->from('tbl_entity as E, sports_contest as C');
        $this->db->where("C.ContestID", "E.EntityID", FALSE);
        $this->db->where("C.ContestDuration !=", "SeasonLong");
        $this->db->where("C.AuctionStatusID", 5);
            ...
            foreach ($getRunningContest as $key => $RunContVal) {
                ...
                if($RunContVal['ContestDuration'] == 'Weekly'){
                    ...
                    if(strtotime($RunContVal['DailyDate']) < strtotime(date('Y-m-d'))){
                        ...
                        $CompleteMatchData = $this->Sports_football_model->getMatches('MatchID,Status,WeekID,MatchStartDateTime,MatchStartDateTimeEST', array('WeekID' => $RunContVal['WeekID'],'MatchStartDateTime'=>$RunContVal['DailyDate'], 'StatusID' => array(1,2), 'OrderBy' => 'MatchStartDateTime', 'Sequence' => 'DESC'), FALSE, '', '');
                        if (empty($CompleteMatchData)) {
    ...
    function getGamePlayers($Field = '', $Where = array(), $multiRecords = FALSE, $PageNo = 1, $PageSize = 15) {
        $Params = array();
        if (!empty($Field)) {
            $Params = array_map('trim', explode(',', $Field));
            $Field = '';
        ...
        if (!empty($Field))
        $this->db->select($Field, FALSE);
        $this->db->from('tbl_entity E, sports_players P,sports_teams T,sports_team_players TP');
        $this->db->where("P.PlayerID", "E.EntityID", FALSE);
        $this->db->where("T.TeamID", "P.TeamID", FALSE);
        $this->db->where("TP.PlayerID", "P.PlayerID", FALSE);
    ...
    function getMatchIds($MatchDate){
