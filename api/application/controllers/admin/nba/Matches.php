...
class Matches extends API_Controller_Secure {
    ...
    public function getMatches_post() {
        ...
        /* Get Matches Data */
        $MatchesData = $this->Sports_model->getMatches(@$this->Post['Params'], array_merge($this->Post, (!empty($this->Post['SeriesGUID'])) ? array('SeriesID' => $this->SeriesID) : array()), TRUE, @$this->Post['PageNo'], @$this->Post['PageSize']);
    ...
    public function updatePlayerInfo_post() {
        ...
        $this->form_validation->set_rules('MatchGUID', 'MatchGUID', 'trim|required|callback_validateEntityGUID[Matches,MatchID]');
        $this->form_validation->set_rules('PlayerRole', 'PlayerRole', 'trim|required|in_list[PointGuard,Center,SmallForward,ShootingGuard,PowerForward]');
        ...
        if (!empty($this->Post['MediaGUIDs'])) {
            ...
            foreach ($MediaGUIDsArray as $MediaGUID) {
                ...
                if ($EntityData) {
                    ...
                    $this->db->query('UPDATE nba_sports_players AS P, tbl_media AS M SET P.PlayerPic = M.MediaName WHERE M.EntityID = P.PlayerID AND M.MediaID = ' . $EntityData['MediaID']);
                }
            }
        }
        $this->Return['Data'] = $this->Sports_model->getPlayers('PlayerSalaryCredit,TeamGUID,TeamName,TeamNameShort,TeamFlag,PlayerID,PlayerIDLive,PlayerRole,IsPlaying,PlayerSalary,PlayerPic,PlayerCountry,PlayerBattingStyle,PlayerBowlingStyle,PlayerBattingStats,PlayerBowlingStats', array('PlayerID' => $this->PlayerID, 'MatchID' => $this->MatchID), FALSE, 0);
