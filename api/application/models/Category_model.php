	...
	function getCategories($Field='', $Where=array(), $multiRecords=FALSE,  $PageNo=1, $PageSize=10){
		...
		if($Query->num_rows()>0){
			foreach($Query->result_array() as $Record){
				...
				/*get attached media*/
				$MediaData = $this->Media_model->getMedia('E.EntityGUID MediaGUID, CONCAT("'.BASE_URL.'",MS.SectionFolderPath,"110_",M.MediaName) AS MediaThumbURL,	CONCAT(MS.SectionFolderPath,M.MediaName) AS MediaURL,	M.MediaCaption',
					array("SectionID" => 7,"EntityID" => $Record['CategoryIDForUse']), TRUE);
