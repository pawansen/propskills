...
class SnakeDrafts_model extends CI_Model {
    ...
    function addContest($Input = array(), $SessionUserID, $MatchID, $SeriesID, $StatusID = 1) {
        ...
        /** user waller amount deduct * */
        if ($this->Post['Privacy'] == 'Yes' && $this->Post['AllowPrivateContestFree'] == 'Yes') {
            $ContestEntryRemainingFees = @$Input['TotalContestFee'];
    ...
    function getPlayersMyTeam($Field = '', $Where = array(), $multiRecords = FALSE, $PageNo = 1, $PageSize = 15) {
        ...
        $this->db->select('P.PlayerGUID,P.PlayerName,C.ContestSize');
        if (!empty($Field))
        $this->db->select($Field, FALSE);
    ...
    function draftPlayerSold($Input = array(), $SeriesID, $ContestID, $UserID, $PlayerID = "") {
        ...
        $DraftGames = $this->getContests('ContestID,SeriesID,SeriesGUID,DraftTotalRounds,TotalJoined,DraftLiveRound,RosterSize,WeekStart', array('AuctionStatusID' => 2, 'LeagueType' => "Draft", "ContestID" => $ContestID, "SeriesID" => $SeriesID), TRUE, 1);
            ...
            if (empty($PlayerID)) {
                ...
                if ($diffSeconds >= $UserTimer) {
                    ...
                    if ($Query->num_rows() > 0) {
                        ...
                        /*$this->db->select("BS.PlayerStatus,BS.TeamID,ST.TeamName,ST.FantasyPoints");
                        $this->db->from('tbl_auction_player_bid_status BS,sports_teams ST');
                        $this->db->where("BS.TeamID", "ST.TeamID", FALSE);
                        $this->db->where_in("BS.PlayerStatus", array("Live", "Upcoming"));
                        $this->db->where("BS.ContestID", $ContestID);
                        $this->db->where("BS.SeriesID", $SeriesID);
                        $this->db->order_by("ST.FantasyPoints", "DESC");
    ...
    function getContests($Field = '', $Where = array(), $multiRecords = FALSE, $PageNo = 1, $PageSize = 15) {
        ...
        $this->db->select('C.ContestGUID,C.ContestName,S.SeriesID,C.ContestID');
        if (!empty($Field))
            $this->db->select($Field, FALSE);
        ...
        if (!empty($Where['Keyword'])) {
            if (is_array(json_decode($Where['Keyword'], true))) {
                ...
                if (isset($Where['Keyword']['LeagueJoinTime'])) {
                    $this->db->where("TIME(C.LeagueJoinDateTime)", @$Where['Keyword']['LeagueJoinTime']);
                }
            ...
            if ($multiRecords) {
                ...
                foreach ($Query->result_array() as $key => $Record) {
                    ...
                    if (isset($Where['WeekInfo']) && $Where['WeekInfo'] == "Yes") {
                        ...
                        $Temp = array();
                        /** to check auction user final team submitted * */
                        for ($i = $Record['WeekStart']; $i <= $Record['WeekEnd']; $i++) {
                            $Temp['Week'] = $i;
                            $this->db->select("UserTeamID,IsWeekCompleted");
                            $this->db->from('sports_users_teams');
                            $this->db->where("ContestID", $Record['ContestID']);
                            $this->db->where("WeekID", $i);
                            $this->db->where("IsPreTeam", "No");
                            $this->db->where("IsAssistant", "No");
                            $this->db->where("AuctionTopPlayerSubmitted", "Yes");
                            $this->db->limit(1);
                            $Query = $this->db->get();
                            if ($Query->num_rows() > 0) {
                                $WeekDetails = $Query->row_array();
                                $Temp["SubmitTeam"] = "Yes";
                                $Temp["IsWeekCompleted"] = $WeekDetails['IsWeekCompleted'];
                            } else {
                                $Temp["SubmitTeam"] = "No";
                                $Temp["IsWeekCompleted"] = "No";
                            }
                            $Records[$key]['WeekTeamInfo'][] = $Temp;
                        }
                    }
                }
                ...
                if (isset($Where['WeekInfo']) && $Where['WeekInfo'] == "Yes") {
                    ...
                    if (!empty($Where['WeekID'])) {
                        $Record['WeekStart'] = $Where['WeekID'];
                        $Record['WeekEnd'] = $Where['WeekID'];
                    }
                    /** to check auction user final team submitted * */
                    for ($i = $Record['WeekStart']; $i <= $Record['WeekEnd']; $i++) {
                        $Temp['Week'] = $i;
            ...
            for ($i = 1; $i <= $DraftTotalRounds; $i++) {
                ...
                foreach ($AllJoinedUsers['Data']['Records'] as $Rows) {
                    $this->db->select('UT.UserID,UT.UserTeamID,UTP.PlayerID,UTP.DateTime,T.PlayerGUID,
                        T.PlayerName,T.PlayerPic,T.PlayerRole,
                        (CASE T.PlayerRole
                             when "QuarterBack" then "QB"
                             when "RunningBack" then "RB"
                             when "WideReceiver" then "WR"
...
//            }
            ...
            if ($multiRecords) {
                ...
                foreach ($Return['Data']['Records'] as $key => $record) {
                    if (!empty($record['UserTeamID'])) {
                        ...
                        $UserTeamPlayers =$this->getPlayersMyTeam('PlayerRoleShort,TeamName,UserTeamGUID,PlayerID,UserTeamID,TotalPoints,PlayerPic', array('ContestID' => $Where['ContestID'],'SessionUserID' => $record['UserID'],'MySquadPlayer'=>'Yes','IsPreTeam'=>'No','PlayerBidStatus'=>'Yes'), true, 0);
    ...
    function contestUserLeaderboard($Field = '', $Where = array(), $multiRecords = FALSE, $PageNo = 1, $PageSize = 15) {
        ...
        if (!empty($Field)) {
            ...
            $Field = '';
            $FieldArray = array(
                'TotalPoints' => '(SELECT SUM(TotalPoints) FROM sports_users_teams WHERE UserID=UT.UserID AND ContestID=UT.ContestID) TotalPoints',
                'WeekTotalPoints' => 'UT.TotalPoints WeekTotalPoints',
                'UserRank' => 'UT.Rank UserRank',
                'FirstName' => 'U.FirstName',
                'WeekID' => 'UT.WeekID',
                'MiddleName' => 'U.MiddleName',
                'LastName' => 'U.LastName',
                ...
                'Loss' => 'UT.Loss',
                'Tie' => 'UT.Tie',
                'SeasonWin' => '(SELECT SUM(Win) FROM sports_users_teams WHERE UserID=UT.UserID AND ContestID=UT.ContestID) SeasonWin',
                'SeasonLoss' => '(SELECT SUM(Loss) FROM sports_users_teams WHERE UserID=UT.UserID AND ContestID=UT.ContestID) SeasonLoss',
                'SeasonTie' => '(SELECT SUM(Tie) FROM sports_users_teams WHERE UserID=UT.UserID AND ContestID=UT.ContestID) SeasonTie'
            );
            if ($Params) {
                foreach ($Params as $Param) {
                    $Field .= (!empty($FieldArray[$Param]) ? ',' . $FieldArray[$Param] : '');
        ...
        $this->db->select('U.UserGUID,UT.UserTeamID');
        if (!empty($Field))
            $this->db->select($Field, FALSE);
        $this->db->from('sports_users_teams UT, tbl_users U');
        $this->db->where("UT.UserID", "U.UserID", FALSE);
        $this->db->where("UT.IsPreTeam", "No");
        ...
        if (!empty($Where['OrderBy']) && !empty($Where['Sequence'])) {
            ...
            if (!empty($Where['SessionUserID'])) {
                $this->db->order_by('UT.UserID=' . $Where['SessionUserID'] . ' DESC', null, FALSE);
            }
            $this->db->order_by('UT.Rank', 'ASC');
        }
        /* Total records count only if want to get multiple records */
        if ($multiRecords) {
            $TempOBJ = clone $this->db;
    ...
    function removeDraftPlayer($ContestID, $SessionUserID, $UserTeamID, $PlayerID) {
    ...
    function addDraftPlayer($ContestID, $SessionUserID, $UserTeamID, $PlayerID) {
