	...
	public function editTeam_post()
	{
		...
		if(!empty($this->Post['MediaGUIDs'])){
			...
			foreach($MediaGUIDsArray as $MediaGUID){
				...
				$this->Sports_model->updateTeamFlag($this->TeamID,array('TeamFlag'=>$MediaData['MediaName']));
			}
		}else if(!empty($this->Post['TeamStats'])){
			$StatsData = $this->Post['TeamStats'];

			$TeamStats['FantasyPts'] 			= $StatsData[0];
			$TeamStats['TotalPoints'] 			= $StatsData[1];
			$TeamStats['Wins'] 					= $StatsData[2];
			$TeamStats['Games'] 				= $StatsData[3];
			$TeamStats['PointDifference'] 		= $StatsData[4];
			$TeamStats['TotalOffensiveYards'] 	= $StatsData[5];
			$TeamStats['DefensiveYardsAllowed'] = $StatsData[6];
	
			$this->Sports_model->updateTeamFlag($this->TeamID,array('FantasyPoints'=> $TeamStats['FantasyPts'],'TeamStats'=> json_encode($TeamStats)));
		}else if (!empty($this->Post['ByeWeek'])) {
			$ByeWeek = implode(",", $this->Post['ByeWeek']);
			$this->Sports_model->updateTeamFlag($this->TeamID,array('ByeWeek'=> $ByeWeek));
		}else if (!empty($this->Post['IsPowerTeam'])) {
			$this->Sports_model->updateTeamFlag($this->TeamID,array('IsPowerTeam'=> $this->Post['IsPowerTeam']));
		}
