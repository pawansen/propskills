app.controller('PageController', function ($scope, $http,$timeout){
    ...
    $scope.getList = function ()
    {
        ...
        $http.post(API_URL+'nba/sports/getSeries', data, contentType).then(function(response) {
            var response = response.data;
    ...
    $scope.loadFormEdit = function (Position, SeriesGUID)
    {
        ...
        $scope.templateURLEdit = PATH_TEMPLATE+module+'/series/edit_form.htm?'+Math.random();
        $scope.data.pageLoading = true;
        $http.post(API_URL+'admin/nba/series/getSeriesDetails','SeriesGUID='+SeriesGUID+'&Params=SeriesName,SeriesGUID,AuctionDraftIsPlayed,Status&SessionKey='+SessionKey, contentType).then(function(response) {
            var response = response.data;
    ...
    $scope.editData = function() {
        ...
        $http.post(API_URL + 'admin/nba/series/changeStatus', data, contentType).then(function(response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
