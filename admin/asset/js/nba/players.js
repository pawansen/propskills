app.controller('PageController', function ($scope, $http, $timeout) {
    ...
    $scope.getList = function ()
    {
        ...
        var data = 'SessionKey=' + SessionKey + '&MatchGUID=' + MatchGUID + '&Params=' + PlayerRole + ',PlayerPic,&PageNo=' + $scope.data.pageNo + '&PageSize=' + $scope.data.pageSize+'&'+$('#filterForm').serialize();
        $http.post(API_URL + 'nba/sports/getPlayers', data, contentType).then(function (response) {
            var response = response.data;
    ...
    $scope.loadFormEdit = function (Position, PlayerGUID)
    {
        ...
        $scope.templateURLEdit = PATH_TEMPLATE + module + '/players/edit_form.htm?' + Math.random();
        $scope.data.pageLoading = true;
        $http.post(API_URL + 'nba/sports/getPlayer', 'PlayerGUID=' + PlayerGUID + '&Params=PlayerRole,PlayerPic,PlayerCountry,PlayerBornPlace,PlayerBattingStyle,PlayerBowlingStyle,MatchType,MatchNo,MatchDateTime,SeriesName,TeamGUID&SessionKey=' + SessionKey, contentType).then(function (response) {
            var response = response.data;
    ...
    $scope.editData = function ()
    {
        ...
        $http.post(API_URL + 'admin/nba/matches/updatePlayerInfo', data, contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
