...
class Sports_bsktbl_model extends CI_Model {

    public function __construct() {
        parent::__construct();
        $this->load->model('Settings_model');
        $this->load->model('nba/AuctionDrafts_model');
        $this->load->model('nba/SnakeDrafts_model');
    }
    ...
    function getMatches($Field = '', $Where = array(), $multiRecords = FALSE, $PageNo = 1, $PageSize = 15) {
        ...
        if (!empty($Field)) {
            ...
            $FieldArray = array(
                ...
                'TotalUserWinning' => '(select SUM(UserWinningAmount) from nba_sports_contest_join where MatchID = M.MatchID AND E.StatusID=' . (!is_array(@$Where['StatusID'])) ? @$Where['StatusID'] : 2 . ' AND UserID=' . @$Where['UserID'] . ') as TotalUserWinning',
            );
            if ($Params) {
                foreach ($Params as $Param) {
                    $Field .= (!empty($FieldArray[$Param]) ? ',' . $FieldArray[$Param] : '');
    ...
    function getPlayers($Field = '', $Where = array(), $multiRecords = FALSE, $PageNo = 1, $PageSize = 15) {
        ...
        if (array_keys_exist($Params, array('TeamGUID', 'TeamName', 'TeamNameShort', 'TeamFlag', 'PlayerRole', 'IsPlaying', 'TotalPoints', 'PointsData', 'SeriesID', 'MatchID'))) {
            $this->db->from('nba_sports_teams T,nba_sports_matches M, nba_sports_team_players TP,nba_sports_set_match_types SSM');
    ...
    /*
      Description: To get matches live score(BASKETBALL API NFL)
     */
    function getMatchesScoreLiveNbaGoalServeByDate($CronID) {
        ...
        $Dates = array(date('Y-m-d', strtotime('-1 day', strtotime(date('Y-m-d')))),date('Y-m-d'));
        // print_r($Dates);die;
        foreach($Dates as $Date){
            $url = SPORTS_API_URL_GOALSERVE . '/bsktbl/nba-scores?date='.date('d.m.Y',strtotime($Date));
            $xml = simplexml_load_string(file_get_contents($url));
            if (!empty($xml)) {
                for ($i = 0; $i < count($xml->category->match); $i++) {
                    $MatchIDLive = (int) $xml->category->match[$i]['id'];
                    $MatchStatus = (string) $xml->category->match[$i]['status'];
                    $options = array(
                        'table' => ' nba_sports_matches',
                        'select' => 'MatchID,MatchIDLive,MatchStartDateTimeEST',
                        'where' => array(
                    ...
                    if(!empty($MatchData)){
                        ...
                        if(empty($StatusData)){
                            ...
                            if (strtolower($MatchStatus) != "postponed" || strtolower($MatchStatus) != "not started") {
                                ...
                                $LocalTeamScore = (int) $xml->category->match[$i]->hometeam['totalscore'];
                                $VisitorTeamScore = (int) $xml->category->match[$i]->awayteam['totalscore'];
                                $startersLocalArrayCount = count($xml->category->match[$i]->player_stats->hometeam->starters->player);
                                $startersVisitorArrayCount = count($xml->category->match[$i]->player_stats->awayteam->starters->player);
                                $startersPlayers = array();
                                $benchPlayers = array();

                                for ($j = 0; $j < $startersLocalArrayCount; $j++) {

                                    $player_id = (int) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['id'];

                                    $team_id = $localteam_id;
                                    $playername = str_replace( array( '\'', '"', ',' , ';', '<', '>' ), ' ', (string) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['name']);

                                    $startersPlayers['name'] = $playername;
                                    $startersPlayers['pos'] = (string) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['pos'];
                                    $startersPlayers['minutes'] = (int) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['minutes'];
                                    $startersPlayers['field_goals_made'] = (string) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['field_goals_made'];
                                    $startersPlayers['field_goals_attempts'] = (int) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['field_goals_attempts'];
                                    $startersPlayers['threepoint_goals_made'] = (int) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['threepoint_goals_made'];
                                    $startersPlayers['threepoint_goals_attempts'] = (int) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['threepoint_goals_attempts'];
                                    $startersPlayers['freethrows_goals_made'] = (int) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['freethrows_goals_made'];
                                    $startersPlayers['freethrows_goals_attempts'] = (int) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['freethrows_goals_attempts'];
                                    $startersPlayers['offence_rebounds'] = (int) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['offence_rebounds'];
                                    $startersPlayers['defense_rebounds'] = (int) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['defense_rebounds'];
                                    $startersPlayers['total_rebounds'] = (int) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['total_rebounds'];
                                    $startersPlayers['assists'] = (int) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['assists'];
                                    $startersPlayers['steals'] = (int) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['steals'];
                                    $startersPlayers['blocks'] = (int) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['blocks'];
                                    $startersPlayers['turnovers'] = (int) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['turnovers'];
                                    $startersPlayers['personal_fouls'] = (int) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['personal_fouls'];
                                    $startersPlayers['plus_minus'] = (string) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['plus_minus'];
                                    $startersPlayers['points'] = (float) $xml->category->match[$i]->player_stats->hometeam->starters->player[$j]['points'];

                                    $startersPlayersData = json_encode($startersPlayers);
                                    $PlayersScore[$player_id]['starters'] = $startersPlayers;
                                    //$this->matchLiveScoreInsertData($match_id, $team_id, $player_id, $passPlayersData);
                                }

                                for ($j = 0; $j < $startersVisitorArrayCount; $j++) {

                                    $player_id = (int) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['id'];
                                   
                                    $team_id = $visitorteam_id;
                                    $playername = str_replace( array( '\'', '"', ',' , ';', '<', '>' ), ' ', (string) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['name']);

                                    $startersPlayers['name'] = $playername;
                                    $startersPlayers['pos'] = (string) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['pos'];
                                    $startersPlayers['minutes'] = (int) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['minutes'];
                                    $startersPlayers['field_goals_made'] = (string) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['field_goals_made'];
                                    $startersPlayers['field_goals_attempts'] = (int) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['field_goals_attempts'];
                                    $startersPlayers['threepoint_goals_made'] = (int) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['threepoint_goals_made'];
                                    $startersPlayers['threepoint_goals_attempts'] = (int) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['threepoint_goals_attempts'];
                                    $startersPlayers['freethrows_goals_made'] = (int) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['freethrows_goals_made'];
                                    $startersPlayers['freethrows_goals_attempts'] = (int) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['freethrows_goals_attempts'];
                                    $startersPlayers['offence_rebounds'] = (int) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['offence_rebounds'];
                                    $startersPlayers['defense_rebounds'] = (int) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['defense_rebounds'];
                                    $startersPlayers['total_rebounds'] = (int) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['total_rebounds'];
                                    $startersPlayers['assists'] = (int) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['assists'];
                                    $startersPlayers['steals'] = (int) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['steals'];
                                    $startersPlayers['blocks'] = (int) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['blocks'];
                                    $startersPlayers['turnovers'] = (int) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['turnovers'];
                                    $startersPlayers['personal_fouls'] = (int) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['personal_fouls'];
                                    $startersPlayers['plus_minus'] = (string) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['plus_minus'];
                                    $startersPlayers['points'] = (float) $xml->category->match[$i]->player_stats->awayteam->starters->player[$j]['points'];
                                ...
                                $benchLocalArrayCount = count($xml->category->match[$i]->player_stats->hometeam->bench->player);
                                $benchVisitorArrayCount = count($xml->category->match[$i]->player_stats->awayteam->bench->player);
                                for ($j = 0; $j < $benchLocalArrayCount; $j++) {

                                    $player_id = (int) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['id'];
                                    $team_id = $localteam_id;
                                    $playername = str_replace( array( '\'', '"', ',' , ';', '<', '>' ), ' ', (string) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['name']);
                                    $benchPlayers['name'] =  $playername ;
                                    $benchPlayers['pos'] = (string) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['pos'];
                                    $benchPlayers['minutes'] = (int) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['minutes'];
                                    $benchPlayers['field_goals_made'] = (string) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['field_goals_made'];
                                    $benchPlayers['field_goals_attempts'] = (int) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['field_goals_attempts'];
                                    $benchPlayers['threepoint_goals_made'] = (int) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['threepoint_goals_made'];
                                    $benchPlayers['threepoint_goals_attempts'] = (int) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['threepoint_goals_attempts'];
                                    $benchPlayers['freethrows_goals_made'] = (int) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['freethrows_goals_made'];
                                    $benchPlayers['freethrows_goals_attempts'] = (int) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['freethrows_goals_attempts'];
                                    $benchPlayers['offence_rebounds'] = (int) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['offence_rebounds'];
                                    $benchPlayers['defense_rebounds'] = (int) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['defense_rebounds'];
                                    $benchPlayers['total_rebounds'] = (int) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['total_rebounds'];
                                    $benchPlayers['assists'] = (int) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['assists'];
                                    $benchPlayers['steals'] = (int) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['steals'];
                                    $benchPlayers['blocks'] = (int) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['blocks'];
                                    $benchPlayers['turnovers'] = (int) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['turnovers'];
                                    $benchPlayers['personal_fouls'] = (int) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['personal_fouls'];
                                    $benchPlayers['plus_minus'] = (string) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['plus_minus'];
                                    $benchPlayers['points'] = (float) $xml->category->match[$i]->player_stats->hometeam->bench->player[$j]['points'];

                                    $benchPlayersData = json_encode($benchPlayers);
                                    $PlayersScore[$player_id]['bench'] = $benchPlayers;
                                    //$this->matchLiveScoreInsertData($match_id, $team_id, $player_id, "", $rushPlayersData);
                                }
                                for ($j = 0; $j < $benchVisitorArrayCount; $j++) {
                                    $player_id = (int) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['id'];
                                    $team_id = $visitorteam_id;
                                    $playername = str_replace( array( '\'', '"', ',' , ';', '<', '>' ), ' ', (string) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['name']);
                                    $benchPlayers['name'] = $playername;
                                    $benchPlayers['pos'] = (string) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['pos'];
                                    $benchPlayers['minutes'] = (int) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['minutes'];
                                    $benchPlayers['field_goals_made'] = (string) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['field_goals_made'];
                                    $benchPlayers['field_goals_attempts'] = (int) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['field_goals_attempts'];
                                    $benchPlayers['threepoint_goals_made'] = (int) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['threepoint_goals_made'];
                                    $benchPlayers['threepoint_goals_attempts'] = (int) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['threepoint_goals_attempts'];
                                    $benchPlayers['freethrows_goals_made'] = (int) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['freethrows_goals_made'];
                                    $benchPlayers['freethrows_goals_attempts'] = (int) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['freethrows_goals_attempts'];
                                    $benchPlayers['offence_rebounds'] = (int) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['offence_rebounds'];
                                    $benchPlayers['defense_rebounds'] = (int) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['defense_rebounds'];
                                    $benchPlayers['total_rebounds'] = (int) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['total_rebounds'];
                                    $benchPlayers['assists'] = (int) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['assists'];
                                    $benchPlayers['steals'] = (int) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['steals'];
                                    $benchPlayers['blocks'] = (int) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['blocks'];
                                    $benchPlayers['turnovers'] = (int) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['turnovers'];
                                    $benchPlayers['personal_fouls'] = (int) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['personal_fouls'];
                                    $benchPlayers['plus_minus'] = (string) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['plus_minus'];
                                    $benchPlayers['points'] = (float) $xml->category->match[$i]->player_stats->awayteam->bench->player[$j]['points'];
                                ...
                                if (!empty($team_stats)) {
                                    ...
                                    if (!empty($hometeam)) {
                                        $team_id = $localteam_id;
                                        $total_score = $LocalTeamScore;

                                        $field_goals_made['total'] = (int) $xml->category->match[$i]->team_stats->hometeam->field_goals_made['total'];
                                        $field_goals_made['attempts'] = (int) $xml->category->match[$i]->team_stats->hometeam->field_goals_made['attempts'];
                                        $field_goals_made['pct'] = (float) $xml->category->match[$i]->team_stats->hometeam->field_goals_made['pct'];

                                        $threepoint_goals_made['total'] = (int) $xml->category->match[$i]->team_stats->hometeam->threepoint_goals_made['total'];
                                        $threepoint_goals_made['attempts'] = (int) $xml->category->match[$i]->team_stats->hometeam->threepoint_goals_made['attempts'];
                                        $threepoint_goals_made['pct'] = (float) $xml->category->match[$i]->team_stats->hometeam->threepoint_goals_made['pct'];

                                        
                                        $freethrows_goals_made['total'] = (int) $xml->category->match[$i]->team_stats->hometeam->freethrows_goals_made['total'];
                                        $freethrows_goals_made['attempts'] = (int) $xml->category->match[$i]->team_stats->hometeam->freethrows_goals_made['attempts'];
                                        $freethrows_goals_made['pct'] = (float) $xml->category->match[$i]->team_stats->hometeam->freethrows_goals_made['pct'];

                                        $rebounds['total'] = (int) $xml->category->match[$i]->team_stats->hometeam->rebounds['total'];
                                        $rebounds['offence'] = (int) $xml->category->match[$i]->team_stats->hometeam->rebounds['offence'];
                                        $rebounds['defense'] = (int) $xml->category->match[$i]->team_stats->hometeam->rebounds['defense'];

                                        $assists = (float) $xml->category->match[$i]->team_stats->hometeam->assists['total'];
                                        $steals = (float) $xml->category->match[$i]->team_stats->hometeam->steals['total'];
                                        $blocks = (float) $xml->category->match[$i]->team_stats->hometeam->blocks['total'];
                                        $turnovers = (float) $xml->category->match[$i]->team_stats->hometeam->turnovers['total'];
                                        $personal_fouls = (float) $xml->category->match[$i]->team_stats->hometeam->personal_fouls['total'];
                                        ...
                                        $TeamScore['localteam'] = array(
                                            ...
                                            'threepoint_goals_made' => $threepoint_goals_made,
                                            'freethrows_goals_made' => $freethrows_goals_made,
                                            'rebounds' => $rebounds,
                                            'assists' => $assists,
                                            'steals' => $steals,
                                            'blocks' => $blocks,
                                    ...
                                    if (!empty($awayteam)) {
                                        $team_id = $visitorteam_id;
                                        $total_score = $VisitorTeamScore;

                                        $field_goals_made['total'] = (int) $xml->category->match[$i]->team_stats->awayteam->field_goals_made['total'];
                                        $field_goals_made['attempts'] = (int) $xml->category->match[$i]->team_stats->awayteam->field_goals_made['attempts'];
                                        $field_goals_made['pct'] = (float) $xml->category->match[$i]->team_stats->awayteam->field_goals_made['pct'];

                                        $threepoint_goals_made['total'] = (int) $xml->category->match[$i]->team_stats->awayteam->threepoint_goals_made['total'];
                                        $threepoint_goals_made['attempts'] = (int) $xml->category->match[$i]->team_stats->awayteam->threepoint_goals_made['attempts'];
                                        $threepoint_goals_made['pct'] = (float) $xml->category->match[$i]->team_stats->awayteam->threepoint_goals_made['pct'];

                                        
                                        $freethrows_goals_made['total'] = (int) $xml->category->match[$i]->team_stats->awayteam->freethrows_goals_made['total'];
                                        $freethrows_goals_made['attempts'] = (int) $xml->category->match[$i]->team_stats->awayteam->freethrows_goals_made['attempts'];
                                        $freethrows_goals_made['pct'] = (float) $xml->category->match[$i]->team_stats->awayteam->freethrows_goals_made['pct'];

                                        $rebounds['total'] = (int) $xml->category->match[$i]->team_stats->awayteam->rebounds['total'];
                                        $rebounds['offence'] = (int) $xml->category->match[$i]->team_stats->awayteam->rebounds['offence'];
                                        $rebounds['defense'] = (int) $xml->category->match[$i]->team_stats->awayteam->rebounds['defense'];

                                        $assists = (float) $xml->category->match[$i]->team_stats->awayteam->assists['total'];
                                        $steals = (float) $xml->category->match[$i]->team_stats->awayteam->steals['total'];
                                        $blocks = (float) $xml->category->match[$i]->team_stats->awayteam->blocks['total'];
                                        $turnovers = (float) $xml->category->match[$i]->team_stats->awayteam->turnovers['total'];
                                        $personal_fouls = (float) $xml->category->match[$i]->team_stats->awayteam->personal_fouls['total'];
                                ...
                                if (strtolower($MatchStatus) == "final" || strtolower($MatchStatus) == "after over time") {
                                    ...
                                    $this->db->query("UPDATE nba_sports_contest AS C, tbl_entity AS E SET E.StatusID = 2 WHERE E.StatusID = 1 AND C.ContestID = E.EntityID AND E.GameSportsType='Nba' AND C.ContestDuration='Daily'");

                                    foreach($PlayersScore as $Keys=>$PlayerState){
                                        $this->db->query("UPDATE nba_sports_team_players JOIN nba_sports_players ON nba_sports_team_players.PlayerID = nba_sports_players.PlayerID SET `nba_sports_team_players`.`ScoreData` = '".json_encode($PlayerState)."' WHERE `nba_sports_team_players`.`MatchID` ='".$MatchID."' AND `nba_sports_players`.`PlayerIDLive` ='".$Keys."' ");
                                    }
                                }
    ...
    /*
      Description: To get matches live score(BASKETBALL API NFL)
     */
    function getMatchesScoreLiveNbaGoalServe($CronID) {
        return true;
    }
    ...
    function calculatePointsNBA($Points = array(), $ScoreValue) {
    ...
    function getPlayersPointGoalServe($CronID, $MatchIDScore = "") {
        ...
        if (!empty($MatchIDScore)) {
            ...
            $LiveMatches = $this->getMatches('MatchStartDateTime,MatchID,MatchType,MatchScoreDetails,StatusID,IsPlayerPointsUpdated,SeriesID', array('Filter' => 'Yesterday', 'StatusID' => array(5), 'IsPlayerPointsUpdated' => 'No', 'OrderBy' => 'M.MatchStartDateTime', 'Sequence' => 'DESC'), true, 1, 10);
        }
        /* Get Live Matches Data */
            ...
            foreach ($LiveMatches['Data']['Records'] as $Value) {
               ...
               $AllPalyers = array();
                foreach ($Value['MatchScoreDetails']['PlayersScore'] as $PlayerKey=>$PlayerSubValue) {
                    if (isset($PlayerSubValue['starters'])) {
                        $AllPalyers[$PlayerKey]['Points'] = $PlayerSubValue['starters']['points'];
                        $AllPalyers[$PlayerKey]['Rebounds'] = $PlayerSubValue['starters']['total_rebounds'];
                        $AllPalyers[$PlayerKey]['Assists'] = $PlayerSubValue['starters']['assists'];
                        $AllPalyers[$PlayerKey]['Steals'] = $PlayerSubValue['starters']['steals'];
                        $AllPalyers[$PlayerKey]['Blocks'] = $PlayerSubValue['starters']['blocks'];
                        $AllPalyers[$PlayerKey]['TurnOvers'] = $PlayerSubValue['starters']['turnovers'];
                    }

                    if (isset($PlayerSubValue['bench'])) {
                        $AllPalyers[$PlayerKey]['Points'] = $PlayerSubValue['bench']['points'];
                        $AllPalyers[$PlayerKey]['Rebounds'] = $PlayerSubValue['bench']['total_rebounds'];
                        $AllPalyers[$PlayerKey]['Assists'] = $PlayerSubValue['bench']['assists'];
                        $AllPalyers[$PlayerKey]['Steals'] = $PlayerSubValue['bench']['steals'];
                        $AllPalyers[$PlayerKey]['Blocks'] = $PlayerSubValue['bench']['blocks'];
                        $AllPalyers[$PlayerKey]['TurnOvers'] = $PlayerSubValue['bench']['turnovers'];
                    }
                }
                if (empty($AllPalyers)) {
                    continue;
                ...
                foreach($AllPalyers as $PlayerLive=>$ScoreValue){
                    ...
                    if(!empty($ScoreValue)){
                        foreach($PointsConfiguration as $PointScore){
                            if($ScoreValue[$PointScore['PointsTypeGUID']] != ''){
                                $PointsData[] = $this->calculatePointsNBA($PointScore,$ScoreValue[$PointScore['PointsTypeGUID']]);
                            }else{
                    ...
                    if(!empty($PointsData)){
                        ...
                        $Query = $this->db->query('SELECT P.PlayerID FROM nba_sports_players P,tbl_entity E WHERE P.PlayerID=E.EntityID AND P.PlayerIDLive = ' . $PlayerLive . ' LIMIT 1');
                        $PlayerID = ($Query->num_rows() > 0) ? $Query->row()->PlayerID : FALSE;
                        ...
                        $this->db->where('PlayerID', $PlayerID);
                        $this->db->limit(1);
                        $this->db->update('nba_sports_team_players', $UpdateData);
                    }
                }
                ...
                if ($Value['StatusID'] == 5) {
                    $this->db->where('MatchID', $Value['MatchID']);
                    $this->db->limit(1);
                    $this->db->update('nba_sports_matches', array('IsPlayerPointsUpdated' => 'Yes'));
                }
            }
        }
        return true;
    ...
    */
    ...
    function contestUserTeamCalculetePointsGoalServe($CronID) {
        ini_set('max_execution_time', 300);
        /* Get Matches Live */
        $LiveMatcheContest = $this->SnakeDrafts_model->getContests('ContestID,ContestDuration,MatchID', array('StatusID' => 2,"GameSportsType" => "Nba"), true, 0);
        ...
        foreach ($LiveMatcheContest['Data']['Records'] as $Value) {
            ...
            if($Value['ContestDuration'] == 'Daily'){
                $MatchesData = $this->getMatches('MatchID', array("GameSportsType" => "Nba","MatchID" => $Value['MatchID'], "StatusID" => array(2,5)),true,0);
            }else{
                $MatchesData = $this->getMatches('MatchID', array("GameSportsType" => "Nba","MatchID" => $Value['MatchID'], "StatusID" => array(2,5)),true,0);  
            }
            ...
            if (!empty($Contests['Data']['Records'])) {
                ...
                foreach ($Contests['Data']['Records'] as $Row) {
                    ...
                    foreach ($UserTeamPlayers['Data']['Records'] as $UserTeamValue) {
                        $this->db->select('SUM(TotalPoints) as TotalPoint,PointsData');
                        ...
                        $this->db->where('PlayerID', $UserTeamValue['PlayerID']);
                        $this->db->limit(1);
                        $this->db->update('nba_sports_users_team_players', array('Points' => $Points,'PointsData' => $Data['PointsData']));
                    }
                    /* Update Player Total Points */
                    $this->db->where('UserID', $Row['UserID']);
                    $this->db->where('ContestID', $Row['ContestID']);
                    $this->db->limit(1);
                    $this->db->update('nba_sports_contest_join', array('TotalPoints' => $TotalPointsSum, 'ModifiedDate' => date('Y-m-d H:i:s')));
                }
            }
    ...
    function updateRankByContestDraft($ContestID) {
        if (!empty($ContestID)) {
            $query = $this->db->query("SELECT FIND_IN_SET( TotalPoints, 
                         ( SELECT GROUP_CONCAT( TotalPoints ORDER BY TotalPoints DESC)
                         FROM nba_sports_contest_join WHERE nba_sports_contest_join.ContestID = '" . $ContestID . "')) AS UserRank,ContestID,UserTeamID,nba_sports_contest_join.UserID
                         FROM nba_sports_contest_join,tbl_users 
                         WHERE nba_sports_contest_join.ContestID = '" . $ContestID . "' AND tbl_users.UserID = nba_sports_contest_join.UserID
                     ");
            $results = $query->result_array();
            if (!empty($results)) {
                $this->db->trans_start();
                foreach ($results as $rows) {
                    ...
                    $this->db->where('UserID', $rows['UserID']);
                    $this->db->limit(1);
                    $this->db->update('nba_sports_contest_join', array('UserRank' => $rows['UserRank']));
                }
                $this->db->trans_complete();
            }
        }
    ...
    function setContestWinners() {
        ...
        ini_set('max_execution_time', 300);
        $this->db->select("SeriesID,ContestID,DailyDate,ContestDuration,WinningAmount,NoOfWinners,EntryFee,CustomizeWinning");
        ...
        if ($Contests) {
            foreach ($Contests as $Value) {
                ...
                if (empty($JoinedContestsUsers)){
                    ...
                    $this->db->where('ContestID', $Value['ContestID']);
                    $this->db->limit(1);
                    $this->db->update('nba_sports_contest', array('IsWinningDistributed' => 'Yes'));
                    continue;
                }
                ...
                if (!empty($OptionWinner)) {
                    foreach ($OptionWinner as $WinnerValue) {
                        ...
                        $this->db->where('ContestID', $Value['ContestID']);
                        $this->db->limit(1);
                        $this->db->update('nba_sports_contest_join', array('UserWinningAmount' => $WinnerValue['UserWinningAmount'], 'ModifiedDate' => date('Y-m-d H:i:s')));
            ...
            foreach ($Contests as $Value) {
                ...
                if ($Query->num_rows() > 0) {
                    ...
                    foreach ($JoinedContestsUsers as $WinnerValue) {
                        ...
                        $this->db->where('ContestID', $Value['ContestID']);
                        $this->db->limit(1);
                        $this->db->update('nba_sports_contest_join', array('IsWinningDistributeAmount' => "Yes", 'ModifiedDate' => date('Y-m-d H:i:s')));
                    ...
                    $this->db->where('ContestID', $Value['ContestID']);
                    $this->db->limit(1);
                    $this->db->update('nba_sports_contest', array('IsWinningDistributeAmount' => "Yes"));
                } else {
                    /* Update Contest Winning Status Yes */
                    $this->db->where('ContestID', $Value['ContestID']);
                    $this->db->limit(1);
    ...
    /*
      Description: To set matches data (BASKETBALL API NBA)
     */

    function getMatchesLiveNbaGoalserve($CronID) {
        $return['code'] = 200;
        $return['response'] = new stdClass();
        $url = SPORTS_API_URL_GOALSERVE . '/bsktbl/nba-shedule';
        $xml = simplexml_load_string(file_get_contents($url));
        if (!empty($xml)) {
            $MatchTypesData = $this->getMatchTypes();
            $MatchTypeIdsData = array_column($MatchTypesData, 'MatchTypeID', 'MatchTypeName');
                $seriesId = (int)$xml['id'][0];
                $seriesName = (string) trim($xml['league'][0]);
                $SeriesIDLive = trim($seriesId);
                $Query = $this->db->query("SELECT S.SeriesID FROM nba_sports_series S,tbl_entity E WHERE S.SeriesID=E.EntityID AND S.SeriesIDLive = '" . $SeriesIDLive . "' AND E.GameSportsType='Nba' LIMIT 1");
                $SeriesID = ($Query->num_rows() > 0) ? $Query->row()->SeriesID : false;
                if (empty($SeriesID)) {
                    /* Add series to entity table and get EntityID. */
                    $SeriesGUID = get_guid();
                    $SeriesID = $this->Entity_model->addEntity($SeriesGUID, array("EntityTypeID" => 7, "StatusID" => 2, "GameSportsType" => "Nba"));
                    $SeriesData = array_filter(array(
                        'SeriesID' => $SeriesID,
                        'SeriesGUID' => $SeriesGUID,
                        'SeriesIDLive' => $SeriesIDLive,
                        'SeriesYear' => date('Y'),
                        'SeriesName' => $seriesName .' '. date('Y'),
                        'AuctionDraftIsPlayed' => 'No'
                    ));
                    $this->db->insert('nba_sports_series', $SeriesData);
                }
                for ($p = 0; $p < count($xml->matches); $p++) {
                    for ($k = 0; $k < count($xml->matches[$p]->match); $k++) {
                        
                        $SeasonTypeLive = (string) $xml->matches[$p]['seasonType'];
                        $SeasonType = "ProBasketballRegularSeasonOwners";
                        if($SeasonTypeLive == "Post Season"){
                            $SeasonType = "ProBasketballPlayoffs";
                        }
                        $match_time = (string) $xml->matches[$p]->match[$k]['time'];
                        $MatchIDLive = (int) $xml->matches[$p]->match[$k]['id'];
                        $match_date = (string) $xml->matches[$p]->match[$k]['formatted_date'];
                        $match_date = date('Y-m-d', strtotime($match_date));
                        $match_time = date('H:i', strtotime($match_time));
                        $match_date_time = $match_date . ' ' . $match_time;
                        $localteam_id_live = (int) $xml->matches[$p]->match[$k]->hometeam['id'];
                        $visitorteam_id_live = (int) $xml->matches[$p]->match[$k]->awayteam['id'];
                        $localteam_name = (string) $xml->matches[$p]->match[$k]->hometeam['name'];
                        $visitorteam_name = (string) $xml->matches[$p]->match[$k]->awayteam['name'];
                        $match_status = strtolower((string) $xml->matches[$p]->match[$k]['status']);
                        $ExplodeLocalTeam = explode(" ",$localteam_name);
                        $ExplodeVisitorTeam = explode(" ",$visitorteam_name);
                        if ($match_status == "not started") {
                            if (!empty($localteam_id_live) && !empty($visitorteam_id_live)) {
                        
                                /* To check if local team is already exist */
                                $Query = $this->db->query("SELECT T.TeamID FROM nba_sports_teams T,tbl_entity E WHERE T.TeamID=E.EntityID AND T.TeamIDLive = " . $localteam_id_live . " AND E.GameSportsType='Nba' LIMIT 1");
                                $TeamIDLocal = ($Query->num_rows() > 0) ? $Query->row()->TeamID : false;
                                if (!$TeamIDLocal) {
                                    /* Add team to entity table and get EntityID. */
                                    $TeamGUID = get_guid();
                                    $TeamIDLocal = $this->Entity_model->addEntity($TeamGUID, array("EntityTypeID" => 9, "StatusID" => 2, "GameSportsType" => "Nba"));
                                    $TeamData = array(
                                        'TeamID' => $TeamIDLocal,
                                        'TeamGUID' => $TeamGUID,
                                        'TeamIDLive' => $localteam_id_live,
                                        'TeamName' => $localteam_name,
                                        'TeamNameShort' => end($ExplodeLocalTeam)
                                    );
                                    $this->db->insert('nba_sports_teams', $TeamData);
                                }

                                /* To check if local team is already exist */
                                $Query = $this->db->query("SELECT T.TeamID FROM nba_sports_teams T,tbl_entity E WHERE T.TeamID=E.EntityID AND T.TeamIDLive = " . $visitorteam_id_live . " AND E.GameSportsType='Nba' LIMIT 1");
                                $TeamIDVisitor = ($Query->num_rows() > 0) ? $Query->row()->TeamID : false;
                                if (!$TeamIDVisitor) {
                                    /* Add team to entity table and get EntityID. */
                                    $TeamGUID = get_guid();
                                    $TeamIDVisitor = $this->Entity_model->addEntity($TeamGUID, array("EntityTypeID" => 9, "StatusID" => 2, "GameSportsType" => "Nba"));
                                    $TeamData = array(
                                        'TeamID' => $TeamIDVisitor,
                                        'TeamGUID' => $TeamGUID,
                                        'TeamIDLive' => $visitorteam_id_live,
                                        'TeamName' => $visitorteam_name,
                                        'TeamNameShort' => end($ExplodeVisitorTeam)
                                    );
                                    $this->db->insert('nba_sports_teams', $TeamData);
                                }
                                /* To check if match is already exist */
                                $Query = $this->db->query('SELECT M.MatchID,E.StatusID FROM nba_sports_matches M,tbl_entity E WHERE M.MatchID = E.EntityID AND M.MatchIDLive = ' . $MatchIDLive . ' AND E.GameSportsType="Nba" LIMIT 1');
                                $MatchID = ($Query->num_rows() > 0) ? $Query->row()->MatchID : false;
                                if (!$MatchID) {
                                    $MatchGUID = get_guid();
                                    $MatchID = $this->Entity_model->addEntity($MatchGUID, array("EntityTypeID" => 8, "StatusID" => 1, "GameSportsType" => "Nba"));
                                    $MatchesAPIData = array(
                                        'MatchID' => $MatchID,
                                        'MatchGUID' => $MatchGUID,
                                        'MatchIDLive' => $MatchIDLive,
                                        'SeriesID' => $SeriesID,
                                        'MatchTypeID' => $MatchTypeIdsData['NBA'],
                                        'MatchNo' => 1,
                                        'TeamIDLocal' => $TeamIDLocal,
                                        'TeamIDVisitor' => $TeamIDVisitor,
                                    ...
                                    $this->db->insert('nba_sports_matches', $MatchesAPIData);
                                }else{


                                    $this->db->where('MatchID', $MatchID);
                                    $this->db->limit(1);
                                    $this->db->update('nba_sports_matches', array('MatchStartDateTimeEST' => date('Y-m-d', strtotime($match_date)),
                                        'MatchStartDateTime' => $match_date_time,'TeamIDLocal'=> $TeamIDLocal,'TeamIDVisitor'=>$TeamIDVisitor));
                                

                                    $MatchTeamDataLocal = array();
                                    $MatchTeamDataVisitor = array();
                                    /** local team data **/
                                    $Query = $this->db->query('SELECT P.PlayerID,P.PlayerRole FROM nba_sports_players P,tbl_entity E WHERE P.PlayerID=E.EntityID AND P.TeamID = ' . $TeamIDLocal . ' AND E.GameSportsType="Nba"');
                                    $Players = ($Query->num_rows() > 0) ? $Query->result_array() : FALSE;
                                    if(!empty($Players)){
                                        foreach($Players as $Player){
                                            $Query = $this->db->query('SELECT T.PlayerID,T.PlayerRole FROM nba_sports_team_players T WHERE T.PlayerID='.$Player['PlayerID'].' AND T.MatchID='.$MatchID.' AND T.TeamID='.$TeamIDLocal.'');
                                            if($Query->num_rows() <= 0){
                                                $MatchTeamDataLocal[] = array(
                                                    'PlayerID' => $Player['PlayerID'],
                                                    'PlayerRole' => $Player['PlayerRole'],
                                                ...
                                                $this->db->where('PlayerID', $Player['PlayerID']);
                                                $this->db->limit(1);
                                                $this->db->update('nba_sports_team_players', array('TeamID' => $TeamIDLocal));  
                                            }
                                        }
                                        if(!empty($MatchTeamDataLocal)){
                                            $this->db->insert_batch('nba_sports_team_players', $MatchTeamDataLocal);  
                                        }
                                        
                                    }

                                    /** visitor team data **/
                                    $Query = $this->db->query('SELECT P.PlayerID,P.PlayerRole  FROM nba_sports_players P,tbl_entity E WHERE P.PlayerID=E.EntityID AND P.TeamID = ' . $TeamIDVisitor . ' AND E.GameSportsType="Nba"');
                                    $Players = ($Query->num_rows() > 0) ? $Query->result_array() : FALSE;

                                    if(!empty($Players)){
                                        foreach($Players as $Player){
                                            $Query = $this->db->query('SELECT T.PlayerID,T.PlayerRole FROM nba_sports_team_players T WHERE T.PlayerID='.$Player['PlayerID'].' AND T.MatchID='.$MatchID.' AND T.TeamID='.$TeamIDVisitor.'');
                                            if($Query->num_rows() <= 0){
                                                $MatchTeamDataVisitor[] = array(
                                                    'PlayerID' => $Player['PlayerID'],
                                                    'PlayerRole' => $Player['PlayerRole'],
                                                ...
                                                $this->db->where('PlayerID', $Player['PlayerID']);
                                                $this->db->limit(1);
                                                $this->db->update('nba_sports_team_players', array('TeamID' => $TeamIDVisitor)); 
                                            }
                                        }
                                        if(!empty($MatchTeamDataVisitor)){
                                            $this->db->insert_batch('nba_sports_team_players', $MatchTeamDataVisitor);  
                                        }
                                    }
                                }
                            }
    ...
    /*
      Description: To set players data (BASKETBALL API NBA)
     */
    function getPlayersLiveByTeamNbaGoalserve($CronID) {
        $return['code'] = 200;
        $return['response'] = new stdClass();

        $Query = $this->db->query('SELECT T.TeamID,T.TeamGUID,T.TeamIDLive,T.TeamKey,T.TeamName,T.TeamFlag FROM nba_sports_teams T,tbl_entity E WHERE T.TeamID=E.EntityID AND E.GameSportsType="Nba" order by T.TeamID ASC');
        $Teams = ($Query->num_rows() > 0) ? $Query->result_array() : FALSE;
        if (!$Teams) {
            $this->db->where('CronID', $CronID);
            $this->db->limit(1);
        ...
        if (!empty($Teams)) {
            foreach ($Teams as $rows) {
                ...
                $url1=$url = SPORTS_API_URL_GOALSERVE . '/bsktbl/' . $TeamIDLive . '_rosters';
                $xml = simplexml_load_string(file_get_contents($url));
                if (!empty($xml)) {
                        $team_flag = $xml->image;
                        $team_image = "";
                        ...
                        if (!empty($team_flag) && empty($rows['TeamFlag'])) {
                            ...
                            for ($j = 0; $j < count($xml->player); $j++) {
                                $IsPlayerRoster[] = $PlayerIDLive = (int) $xml->player[$j]['id'];
                                $PlayerName = (string) $xml->player[$j]['name'];
                                $PlayerLiveRole = strtoupper((string) $xml->player[$j]['position']);
                                $player_salarycap = ltrim((string) $xml->player[$j]['salary'], '$');
                                $player_salarycap = str_replace(',', '', $player_salarycap);
                                $pRole = "";
                               
                                /* player role system */
                                $Roles = array('PG' => "PointGuard",'C' => "Center", 'SF' => "SmallForward", 'SG' => "ShootingGuard", 'PF' => "PowerForward");

                                /* To check if player is already exist */
                                $Query = $this->db->query('SELECT P.PlayerID FROM nba_sports_players P,tbl_entity E WHERE P.PlayerID=E.EntityID AND P.PlayerIDLive = ' . $PlayerIDLive . ' AND E.GameSportsType="Nba" LIMIT 1');
                                //$PlayerID = ($Query->num_rows() > 0) ? $Query->row()->PlayerID : FALSE;
                                if ($Query->num_rows() == 0){
                                        $PlayerGUID = get_guid();
                                        $player_image = "";
                                        $url = SPORTS_API_URL_GOALSERVE. '/bsktbl/usa?playerimage=' . $PlayerIDLive;
                                        ...
                                        if(!empty($PlayerRole)){
                                            $PlayerID = $this->Entity_model->addEntity($PlayerGUID, array("EntityTypeID" => 10, "StatusID" => 2, "GameSportsType" => "Nba"));
                                            $PlayersAPIData = array(
                                                'TeamID' => $TeamID,
                                                'PlayerID' => $PlayerID,
                                                'PlayerGUID' => $PlayerGUID,
                                            ...
                                            $this->db->insert('nba_sports_players', $PlayersAPIData);
                                        }
                                }else{
                                        $this->db->where('PlayerIDLive', $PlayerIDLive);
                                        $this->db->limit(1);
                                        $this->db->update('nba_sports_players', array('IsPlayRoster' => 'Yes',
                                        'TeamID' => $TeamID));
                                }
                            }
                        ...
                        if(!empty($IsPlayerRoster)){
                            $this->db->where('TeamID', $TeamID);
                            $this->db->update('nba_sports_players', array('IsPlayRoster' => 'No'));
        ...
        /*
      Description: To set matches data (BASKETBALL API NFL)
     */
    function getPlayersLiveByTeamInjuriesAndStatesNba($CronID) {
        $return['code'] = 200;
        $return['response'] = new stdClass();
        ...
        if (!empty($Teams)) {
            foreach ($Teams as $rows) {
                ...
                $this->db->where('TeamID', $TeamID);
                $this->db->update('nba_sports_players', array('IsInjuries' => "Active"));

                /** update player status **/
                $url = SPORTS_API_URL_GOALSERVE . '/bsktbl/' . $TeamIDLive . '_injuries';
                $xml1 = simplexml_load_string(file_get_contents($url));
                if(!empty($xml1)){
                    for ($i = 0; $i < count($xml1->report); $i++) {
                        $player_id = (int)$xml1->report[$i]['player_id'];
                        $this->db->where('PlayerIDLive', $player_id);
                        $this->db->limit(1);
                        $this->db->update('nba_sports_players', array('IsInjuries' => (string)$xml1->report[$i]['status']));
                    }
                }

                $Query = $this->db->query('SELECT T.PlayerID,T.PlayerIDLive,T.PlayerRole,T.TeamID FROM nba_sports_players T,tbl_entity E WHERE T.PlayerID=E.EntityID AND E.GameSportsType="Nba" AND T.TeamID='.$TeamID.' ');
                $Players = ($Query->num_rows() > 0) ? $Query->result_array() : FALSE;
                $Players = array_column($Players,'PlayerRole','PlayerIDLive');
   
                /** update player status **/
                $url = SPORTS_API_URL_GOALSERVE . '/bsktbl/' . $TeamIDLive . '_stats';
                    ...
                    for ($i = 0; $i < count($xml2->category); $i++) {
                        ...
                        for ($j = 0; $j < count($xml2->category[$i]->player); $j++) {
                                ...
                                $PlayerStats = array_filter(array(
                                    ...
                                    'games_played' => (isset($xml2->category[$i]->player[$j]['games_played'])) ? (string) $xml2->category[$i]->player[$j]['games_played'] : 0,
                                    'games_started' => (isset($xml2->category[$i]->player[$j]['games_started'])) ? (string) $xml2->category[$i]->player[$j]['games_started'] : 0,
                                    'minutes' => isset($xml2->category[$i]->player[$j]['minutes']) ? (string) $xml2->category[$i]->player[$j]['minutes'] : 0,
                                    'points_per_game' => (isset($xml2->category[$i]->player[$j]['points_per_game'])) ? (string) $xml2->category[$i]->player[$j]['points_per_game'] : 0,
                                    'offensive_rebounds_per_game' => (isset($xml2->category[$i]->player[$j]['offensive_rebounds_per_game'])) ? (string) $xml2->category[$i]->player[$j]['offensive_rebounds_per_game'] : 0,
                                    'defensive_rebounds_per_game' => (isset($xml2->category[$i]->player[$j]['defensive_rebounds_per_game'])) ? (string) $xml2->category[$i]->player[$j]['defensive_rebounds_per_game'] : 0,
                                    'rebounds_per_game' => (isset($xml2->category[$i]->player[$j]['rebounds_per_game']))?(string) $xml2->category[$i]->player[$j]['rebounds_per_game']:0,
                                    'assists_per_game' => (isset($xml2->category[$i]->player[$j]['assists_per_game']))?(string) $xml2->category[$i]->player[$j]['assists_per_game']:0,
                                    'steals_per_game' => isset($xml2->category[$i]->player[$j]['steals_per_game'])?(string) $xml2->category[$i]->player[$j]['steals_per_game']:0,
                                    'blocks_per_game' => isset($xml2->category[$i]->player[$j]['blocks_per_game'])?(string) $xml2->category[$i]->player[$j]['blocks_per_game']:0,
                                    'turnovers_per_game' => isset($xml2->category[$i]->player[$j]['turnovers_per_game'])?(string) $xml2->category[$i]->player[$j]['turnovers_per_game']:0,
                                    'fouls_per_game' => isset($xml2->category[$i]->player[$j]['fouls_per_game'])?(string) $xml2->category[$i]->player[$j]['fouls_per_game']:0,
                                    'efficiency_rating' => isset($xml2->category[$i]->player[$j]['efficiency_rating'])?(string) $xml2->category[$i]->player[$j]['efficiency_rating']:0,
                                    'fg_made_per_game' => isset($xml2->category[$i]->player[$j]['fg_made_per_game'])?(string) $xml2->category[$i]->player[$j]['fg_made_per_game']:0,
                                    'fg_attempts_per_game' => isset($xml2->category[$i]->player[$j]['fg_attempts_per_game'])?(string) $xml2->category[$i]->player[$j]['fg_attempts_per_game']:0,
                                    'fg_pct' => isset($xml2->category[$i]->player[$j]['fg_pct'])?(string) $xml2->category[$i]->player[$j]['fg_pct']:0,
                                    'three_point_made_per_game' => isset($xml2->category[$i]->player[$j]['three_point_made_per_game'])?(string) $xml2->category[$i]->player[$j]['three_point_made_per_game']:0,
                                    'three_point_attempts_per_game' => isset($xml2->category[$i]->player[$j]['three_point_attempts_per_game'])?(string) $xml2->category[$i]->player[$j]['three_point_attempts_per_game']:0,
                                    'three_point_pct' => isset($xml2->category[$i]->player[$j]['three_point_pct'])?(string) $xml2->category[$i]->player[$j]['three_point_pct']:0,
                                    'free_throws_made_per_game' => isset($xml2->category[$i]->player[$j]['free_throws_made_per_game'])?(string) $xml2->category[$i]->player[$j]['free_throws_made_per_game']:0,
                                    'free_throws_attempts_per_game' => isset($xml2->category[$i]->player[$j]['free_throws_attempts_per_game'])?(string) $xml2->category[$i]->player[$j]['free_throws_attempts_per_game']:0,
                                    'free_throws_pct' => isset($xml2->category[$i]->player[$j]['free_throws_pct'])?(string) $xml2->category[$i]->player[$j]['free_throws_pct']:0,
                                    'two_point_made_per_game' => isset($xml2->category[$i]->player[$j]['two_point_made_per_game'])?(string) $xml2->category[$i]->player[$j]['two_point_made_per_game']:0,
                                    'two_point_attemps_per_game' => isset($xml2->category[$i]->player[$j]['two_point_attemps_per_game'])?(string) $xml2->category[$i]->player[$j]['two_point_attemps_per_game']:0,
                                    'two_point_pct' => isset($xml2->category[$i]->player[$j]['two_point_pct'])?(string) $xml2->category[$i]->player[$j]['two_point_pct']:0,
                                    'points_per_shot' => isset($xml2->category[$i]->player[$j]['points_per_shot'])?(string) $xml2->category[$i]->player[$j]['points_per_shot']:0,
                                    'field_goal_pct_avg' => isset($xml2->category[$i]->player[$j]['field_goal_pct_avg'])?(string) $xml2->category[$i]->player[$j]['field_goal_pct_avg']:0
                                ));
                                ...
                                if(!empty($PlayerStats)){
                                    ...
                                    if($Category == 'Game'){
                                      $Game['rank'] = (isset($xml2->category[$i]->player[$j]['rank'])) ? (string) $xml2->category[$i]->player[$j]['rank'] : 0 ;
                                      $Game['games_played'] = (isset($xml2->category[$i]->player[$j]['games_played'])) ? (string) $xml2->category[$i]->player[$j]['games_played'] : 0 ;
                                      $Game['games_started'] = (isset($xml2->category[$i]->player[$j]['games_started'])) ? (string) $xml2->category[$i]->player[$j]['games_started'] : 0 ;
                                      $Game['minutes'] = (isset($xml2->category[$i]->player[$j]['minutes'])) ? (string) $xml2->category[$i]->player[$j]['minutes'] : 0 ;
                                      $Game['points_per_game'] = (isset($xml2->category[$i]->player[$j]['points_per_game'])) ? (string) $xml2->category[$i]->player[$j]['points_per_game'] : 0 ;
                                      $Game['offensive_rebounds_per_game'] = (isset($xml2->category[$i]->player[$j]['offensive_rebounds_per_game'])) ? (string) $xml2->category[$i]->player[$j]['offensive_rebounds_per_game'] : 0 ;
                                      $Game['defensive_rebounds_per_game'] = (isset($xml2->category[$i]->player[$j]['defensive_rebounds_per_game'])) ? (string) $xml2->category[$i]->player[$j]['defensive_rebounds_per_game'] : 0 ;
                                      $Game['rebounds_per_game'] = (isset($xml2->category[$i]->player[$j]['rebounds_per_game'])) ? (string) $xml2->category[$i]->player[$j]['rebounds_per_game'] : 0 ;
                                      $Game['assists_per_game'] = (isset($xml2->category[$i]->player[$j]['assists_per_game'])) ? (string) $xml2->category[$i]->player[$j]['assists_per_game'] : 0 ;
                                      $Game['steals_per_game'] = (isset($xml2->category[$i]->player[$j]['steals_per_game'])) ? (string) $xml2->category[$i]->player[$j]['steals_per_game'] : 0 ;
                                      $Game['blocks_per_game'] = (isset($xml2->category[$i]->player[$j]['blocks_per_game'])) ? (string) $xml2->category[$i]->player[$j]['blocks_per_game'] : 0 ;
                                      $Game['turnovers_per_game'] = (isset($xml2->category[$i]->player[$j]['turnovers_per_game'])) ? (string) $xml2->category[$i]->player[$j]['turnovers_per_game'] : 0 ;
                                      $Game['fouls_per_game'] = (isset($xml2->category[$i]->player[$j]['fouls_per_game'])) ? (string) $xml2->category[$i]->player[$j]['fouls_per_game'] : 0 ;
                                      $Game['efficiency_rating'] = (isset($xml2->category[$i]->player[$j]['efficiency_rating'])) ? (string) $xml2->category[$i]->player[$j]['efficiency_rating'] : 0 ;

                                      $PlayerUpdates['Game'] = json_encode($Game);
                                    }

                                     if($Category == 'Shooting'){
                                        $Shooting['rank'] = (isset($xml2->category[$i]->player[$j]['rank'])) ? (string) $xml2->category[$i]->player[$j]['rank'] : 0 ;
                                        $Shooting['fg_made_per_game'] = (isset($xml2->category[$i]->player[$j]['fg_made_per_game'])) ? (string) $xml2->category[$i]->player[$j]['fg_made_per_game'] : 0 ;
                                        $Shooting['fg_attempts_per_game'] = (isset($xml2->category[$i]->player[$j]['fg_attempts_per_game'])) ? (string) $xml2->category[$i]->player[$j]['fg_attempts_per_game'] : 0 ;
                                        $Shooting['fg_pct'] = (isset($xml2->category[$i]->player[$j]['fg_pct'])) ? (string) $xml2->category[$i]->player[$j]['fg_pct'] : 0 ;
                                        $Shooting['three_point_made_per_game'] = (isset($xml2->category[$i]->player[$j]['three_point_made_per_game'])) ? (string) $xml2->category[$i]->player[$j]['three_point_made_per_game'] : 0 ;
                                        $Shooting['three_point_attempts_per_game'] = (isset($xml2->category[$i]->player[$j]['three_point_attempts_per_game'])) ? (string) $xml2->category[$i]->player[$j]['three_point_attempts_per_game'] : 0 ;
                                        $Shooting['three_point_pct'] = (isset($xml2->category[$i]->player[$j]['three_point_pct'])) ? (string) $xml2->category[$i]->player[$j]['three_point_pct'] : 0 ;
                                        $Shooting['free_throws_made_per_game'] = (isset($xml2->category[$i]->player[$j]['free_throws_made_per_game'])) ? (string) $xml2->category[$i]->player[$j]['free_throws_made_per_game'] : 0 ;
                                        $Shooting['free_throws_attempts_per_game'] = (isset($xml2->category[$i]->player[$j]['free_throws_attempts_per_game'])) ? (string) $xml2->category[$i]->player[$j]['free_throws_attempts_per_game'] : 0 ;
                                        $Shooting['free_throws_pct'] = (isset($xml2->category[$i]->player[$j]['free_throws_pct'])) ? (string) $xml2->category[$i]->player[$j]['free_throws_pct'] : 0 ;
                                        $Shooting['two_point_made_per_game'] = (isset($xml2->category[$i]->player[$j]['two_point_made_per_game'])) ? (string) $xml2->category[$i]->player[$j]['two_point_made_per_game'] : 0 ;
                                        $Shooting['two_point_attemps_per_game'] = (isset($xml2->category[$i]->player[$j]['two_point_attemps_per_game'])) ? (string) $xml2->category[$i]->player[$j]['two_point_attemps_per_game'] : 0 ;
                                        $Shooting['two_point_pct'] = (isset($xml2->category[$i]->player[$j]['two_point_pct'])) ? (string) $xml2->category[$i]->player[$j]['two_point_pct'] : 0 ;
                                        $Shooting['points_per_shot'] = (isset($xml2->category[$i]->player[$j]['points_per_shot'])) ? (string) $xml2->category[$i]->player[$j]['points_per_shot'] : 0 ;
                                        $Shooting['field_goal_pct_avg'] = (isset($xml2->category[$i]->player[$j]['field_goal_pct_avg'])) ? (string) $xml2->category[$i]->player[$j]['field_goal_pct_avg'] : 0 ;

                                        $PlayerUpdates['Shooting'] = json_encode($Shooting);
                                    }
                                ...
                                if(!empty($PlayerUpdates)){
                                    ...
                                    $this->db->where('PlayerIDLive', $player_id);
                                    $this->db->limit(1);
                                    $this->db->update('nba_sports_players', $PlayerUpdates); 
                                }
    ...
    function matchWisePlayerStatsUpdates() {

                $MatchID=$this->SnakeDrafts_model->getCurrentMatchLast();
                $Players = array();
                $this->db->select("TP.MatchID,P.PlayerID,P.MatchStats,TP.ScoreData,TP.TotalPoints");
                $this->db->from('nba_sports_team_players TP,nba_sports_players P');
                $this->db->where("P.PlayerID", "TP.PlayerID", FALSE);
                $this->db->where("TP.MatchID",  $MatchID['MatchID']);
                ...
                if(!empty($Players)){
                     foreach($Players as $Player){
                        $MatchStats = array();
                        $MatchStats['total_points'] = $Player['TotalPoints'];
                        $MatchStats['assists_per_game'] = 0;
                        $MatchStats['steals_per_game'] = 0;
                        $MatchStats['blocks_per_game'] = 0;
                        $MatchStats['turnovers_per_game'] = 0;
                        $MatchStats['points_per_game'] = 0;
                        $MatchStats['offensive_rebounds_per_game'] = 0;
                        $MatchStats['defensive_rebounds_per_game'] = 0;
                        $MatchStats['rebounds_per_game'] = 0;
                        $MatchStats['fg_made_per_game'] = 0;
                        $MatchStats['fg_attempts_per_game'] = 0;
                        $MatchStats['three_point_made_per_game'] = 0;
                        $MatchStats['three_point_attempts_per_game'] = 0;
                        $MatchStats['free_throws_made_per_game'] = 0;
                        $MatchStats['free_throws_attempts_per_game'] = 0;
                        $MatchStats['minutes'] = 0;
                      
                        $ScoreData = json_decode($Player['ScoreData'],true);
                       
                        if(isset($ScoreData['bench'])){
                            $MatchStats['assists_per_game'] = $ScoreData['bench']['assists'];
                            $MatchStats['steals_per_game'] = $ScoreData['bench']['steals'];
                            $MatchStats['blocks_per_game'] = $ScoreData['bench']['blocks'];
                            $MatchStats['turnovers_per_game'] = $ScoreData['bench']['turnovers'];
                            $MatchStats['points_per_game'] = $ScoreData['bench']['points'];
                            $MatchStats['offensive_rebounds_per_game'] = $ScoreData['bench']['offence_rebounds'];
                            $MatchStats['defensive_rebounds_per_game'] = $ScoreData['bench']['defense_rebounds'];
                            $MatchStats['rebounds_per_game'] = $ScoreData['bench']['total_rebounds'];
                            $MatchStats['minutes'] = $ScoreData['bench']['minutes'];
                            $MatchStats['fg_made_per_game'] = $ScoreData['bench']['field_goals_made'];
                            $MatchStats['fg_attempts_per_game'] = $ScoreData['bench']['field_goals_attempts'];
                            $MatchStats['three_point_made_per_game'] = $ScoreData['bench']['threepoint_goals_made'];
                            $MatchStats['three_point_attempts_per_game'] = $ScoreData['bench']['threepoint_goals_attempts'];
                            $MatchStats['free_throws_made_per_game'] = $ScoreData['bench']['freethrows_goals_made'];
                            $MatchStats['free_throws_attempts_per_game'] = $ScoreData['bench']['freethrows_goals_attempts'];
                        }
                        if(isset($ScoreData['starters'])){
                            $MatchStats['assists_per_game'] = $ScoreData['starters']['assists'];
                            $MatchStats['steals_per_game'] = $ScoreData['starters']['steals'];
                            $MatchStats['blocks_per_game'] = $ScoreData['starters']['blocks'];
                            $MatchStats['turnovers_per_game'] = $ScoreData['starters']['turnovers'];
                            $MatchStats['points_per_game'] = $ScoreData['starters']['points'];
                            $MatchStats['offensive_rebounds_per_game'] = $ScoreData['starters']['offence_rebounds'];
                            $MatchStats['defensive_rebounds_per_game'] = $ScoreData['starters']['defense_rebounds'];
                            $MatchStats['rebounds_per_game'] = $ScoreData['starters']['total_rebounds'];
                            $MatchStats['minutes'] = $ScoreData['starters']['minutes'];
                            $MatchStats['fg_made_per_game'] = $ScoreData['starters']['field_goals_made'];
                            $MatchStats['fg_attempts_per_game'] = $ScoreData['starters']['field_goals_attempts'];
                            $MatchStats['three_point_made_per_game'] = $ScoreData['starters']['threepoint_goals_made'];
                            $MatchStats['three_point_attempts_per_game'] = $ScoreData['starters']['threepoint_goals_attempts'];
                            $MatchStats['free_throws_made_per_game'] = $ScoreData['starters']['freethrows_goals_made'];
                            $MatchStats['free_throws_attempts_per_game'] = $ScoreData['starters']['freethrows_goals_attempts'];
                        }

                        $this->db->where('PlayerID', $Player['PlayerID']);
                        $this->db->limit(1);
                        $this->db->update('nba_sports_players', array('MatchStats' => json_encode($MatchStats)));
                     }
                }
    }
