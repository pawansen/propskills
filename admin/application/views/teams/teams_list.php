...
<div class="panel-body" ng-controller="PageController"><!-- Body -->
	...
	<div class="clearfix mt-2 mb-2">
		...
		<div class="float-right">
			<form id="filterForm1" role="form" autocomplete="off" ng-submit="applyFilter()" class="ng-pristine ng-valid">
		...
		<table class="table table-striped table-condensed table-hover table-sortable" ng-if="data.dataList.length">
			...
			<thead>
				<tr>
					...
					<th style="width: 100px;">Fantasy Pts</th>
					<th>Total Points</th>
					<th>Wins</th>
					<th>Games</th>
					<th>Point Difference</th>
					<th>Total Offensive Yards</th>
					<th>Defensive Yards Allowed</th>
					<th>Power Play</th>
			...
			<tbody id="tabledivbody">
				<tr scope="row" ng-repeat="(key, row) in data.dataList" id="sectionsid_{{row.MenuOrder}}.{{row.CategoryID}}">
					...
					<td ng-if="row.FantasyPoints">{{row.FantasyPoints}}</td>
					<td ng-if="!row.FantasyPoints">-</td>
					<td ng-if="row.TeamStats.TotalPoints">{{row.TeamStats.TotalPoints}}</td>
					<td ng-if="!row.TeamStats.TotalPoints">-</td>
					<td ng-if="row.TeamStats.Wins">{{row.TeamStats.Wins}}</td>
					<td ng-if="!row.TeamStats.Wins">-</td>
					<td ng-if="row.TeamStats.Games">{{row.TeamStats.Games}}</td>
					<td ng-if="!row.TeamStats.Games">-</td>
					<td ng-if="row.TeamStats.PointDifference">{{row.TeamStats.PointDifference}}</td>
					<td ng-if="!row.TeamStats.PointDifference">-</td>
					<td ng-if="row.TeamStats.TotalOffensiveYards">{{row.TeamStats.TotalOffensiveYards}}</td>
					<td ng-if="!row.TeamStats.TotalOffensiveYards">-</td>
					<td ng-if="row.TeamStats.DefensiveYardsAllowed">{{row.TeamStats.DefensiveYardsAllowed}}</td>
					<td ng-if="!row.TeamStats.DefensiveYardsAllowed">-</td>
					<td><input type="checkbox" name="IsPowerTeam" ng-model="row.IsPowerTeam" ng-if="row.GameSportsType == 'Ncaaf'"  ng-click="editData('PowerFive',row.TeamGUID,row.IsPowerTeam)"></td>
					<td class="text-center">
						<div class="dropdown">
							...
							<div class="dropdown-menu dropdown-menu-left">
								...
								<a class="dropdown-item" href="" ng-click="loadFormUpdate(key, row.TeamGUID)">Stats</a>
							</div>
						</div>
					</td>
				</tr>
	...
	<div class="modal fade" id="update_form">
		<div class="modal-dialog modal-md" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h3 class="modal-title h5">Update Team Stats</h3>     	
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				</div>
