app.controller('PageController', function ($scope, $http,$timeout){
    ...
    $scope.getList = function ()
    {
        ...
        $http.post(API_URL+'admin/nba/AuctionDrafts/getPrivateContest', data, contentType).then(function(response) {
        var response = response.data;
    ...
    $scope.loadContestJoinedUser = function (Position, ContestGUID)
    {
        ...
        $http.post(API_URL + 'snakeDraft/getJoinedContestsUsers', 'SessionKey=' + SessionKey + '&ContestGUID=' + ContestGUID + '&Params=UserTeamName,TotalPoints,UserWinningAmount,Email,PhoneNumber,FirstName,Username,UserGUID,UserTeamPlayers,UserTeamID,UserRank', contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
    ...
    $scope.loadFormStatus = function (Position, ContestGUID)
    {
        ...
        $scope.data.pageLoading = true;
        $http.post(API_URL + 'snakeDraft/getContest', 'SessionKey=' + SessionKey + '&ContestGUID=' + ContestGUID + '&Params=ContestName,ContestType,Status,StatusID', contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
                $scope.data.pageLoading = false;
                $scope.formData = response.Data
