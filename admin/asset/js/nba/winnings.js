app.controller('PageController', function ($scope, $http,$timeout){
    ...
    $scope.getList = function ()
    {
        ...
        var data = 'SessionKey='+SessionKey+'&PageNo=' + $scope.data.pageNo + '&PageSize=' + $scope.data.pageSize+'&Params=Privacy,IsPaid,WinningAmount,ContestSize,EntryFee,NoOfWinners,EntryType,TeamNameLocal,TeamNameVisitor,Status,CustomizeWinning,ContestType,MatchStartDateTime,TotalJoined&Privacy=All&StatusID=5&OrderBy=MatchStartDateTime&Sequence=DESC';
        $http.post(API_URL+'nba/contest/getContests', data, contentType).then(function(response) {
            var response = response.data;
