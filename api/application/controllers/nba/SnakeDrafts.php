...
class SnakeDrafts extends API_Controller {
    ...
    public function getJoinedDraftAllTeams_post() {
        ...
        $AllTeams = $this->SnakeDrafts_model->getJoinedDraftAllTeams($this->ContestID, $this->SeriesID, $this->UserID,$this->Post['DraftHistory']);
    ...
    public function getPlayers_post() {
        ...
        $this->form_validation->validation($this);  /* Run validation */
        /* Get Players Data */
        $PlayersData = $this->SnakeDrafts_model->getPlayers(@$this->Post['Params'], array_merge($this->Post, array('SeriesID' => $this->SeriesID,'MatchID' => @$this->MatchID, 'ContestID' => @$this->ContestID, 'SessionUserID' => @$this->SessionUserID, 'PlayerID' => @$this->PlayerID, 'UserID' => @$this->UserID)), TRUE, @$this->Post['PageNo'], @$this->Post['PageSize']);
    ...
    public function getPlayersDraft_post() {
        ...
        $this->form_validation->validation($this);  /* Run validation */
        /* Get Players Data */
        $PlayersData = $this->SnakeDrafts_model->getPlayersDraftAll(@$this->Post['Params'], array_merge($this->Post, array('SeriesID' => $this->SeriesID,'MatchID' => @$this->MatchID, 'ContestID' => @$this->ContestID, 'SessionUserID' => @$this->UserID, 'PlayerID' => @$this->PlayerID)), TRUE, @$this->Post['PageNo'], @$this->Post['PageSize']);
        if (!empty($PlayersData)) {
            ...
            $Query = $this->db->query('SELECT IsAutoDraft FROM nba_sports_contest_join WHERE ContestID = "' . @$this->ContestID . '" AND UserID = "' . @$this->UserID . '" LIMIT 1');
            $PlayersData['Data']['IsAutoDraft'] = $Query->row()->IsAutoDraft;
            $this->Return['Data'] = $PlayersData['Data'];
        }else{
            $RoosterArray = array();
            if($this->Post['IsPreTeam'] == "No" && $this->Post['IsAssistant'] == "No"){
                ...
                if($ContestsUsers['Privacy'] == "No"){
                    $Roosters = basketballGetConfigurationPlayersRooster($ContestsUsers['ContestSize']);
                }else{
                    $Roosters = basketballGetConfigurationPlayersRoosterPrivate($ContestsUsers['DraftTotalRounds']);
                }
                ...
                $PlayersData['Data']['TotalRecords'] = 0;
            }
    ...
    public function getPlayersMyTeam_post() {
        ...
        if (!empty($PlayersData)) {
            if(!empty($PlayersData['Data']['Records'])){
                ...
                $FlexPlayers = array_filter($PlayersData['Data']['Records'], function ($var) {
                    return ($var['PlayerSelectTypeRole'] == 'FLEX');
                });
                
                $FlexPlayers = array_values($FlexPlayers);
                if($PlayersData['Data']['TotalRecords'] < array_sum($Roosters)){
                    foreach($Roosters as $Key=>$Row){
                        if($RoosterPlayer[$Key] < $Row){

                            if($Key == "FLEX" && $RoosterPlayer[$Key] > 0 && $RoosterPlayer[$Key] < $Row){
                                for($i=0; $i< $RoosterPlayer[$Key]; $i++){
                                    array_push($RoosterArray, $FlexPlayers[$i]);
                                }
                            }
                            for($i=1; $i<= $Row - $RoosterPlayer[$Key]; $i++){
                                $RoosterArray[]['PlayerSelectTypeRole'] = $Key;
                            ...
                            if ($Key == "FLEX") {
                                ...
                                $DataIndex = GetIndexValues($PlayersData['Data']['Records'],'PlayerSelectTypeRole', $Key);
                                array_push($RoosterArray, $PlayersData['Data']['Records'][$DataIndex]);
                            }
                        } 
                    }  
                }else{
    ...
    public function addUserTeam_post() {
        ...
        $UserTeam = $this->SnakeDrafts_model->addUserTeam($this->Post, $this->SessionUserID, $this->SeriesID, $this->MatchID, $this->ContestID);
        if (!$UserTeam) {
            $this->Return['ResponseCode'] = 500;
            $this->Return['Message'] = "An error occurred, please try again later.";
        } else {
    ...
    public function autoDraftOnOff_post() {
        ...
        $this->form_validation->validation($this);  /* Run validation */
        /* Validation - ends */
        $UserTeam = $this->SnakeDrafts_model->autoDraftOnOff($this->Post, $this->SessionUserID, $this->SeriesID, $this->MatchID, $this->ContestID);
    ...
    public function assistantTeamOnOff_post() {
        ...
        $this->form_validation->validation($this);  /* Run validation */
        /* Validation - ends */
        $UserTeam = $this->SnakeDrafts_model->assistantTeamOnOff($this->Post, $this->SessionUserID, $this->SeriesID, $this->MatchID, $this->ContestID, $this->UserTeamID);
    ...
    public function getUserTeams_post() {
        ...
        /* Get User Teams Data */
        $UserTeams = $this->SnakeDrafts_model->getUserTeams(@$this->Post['Params'], array_merge($this->Post, array('UserID' => $this->SessionUserID, 'SeriesID' => $this->SeriesID,'MatchID' => $this->MatchID, 'UserTeamID' => @$this->UserTeamID)), (!empty($this->Post['UserTeamGUID'])) ? FALSE : TRUE, @$this->Post['PageNo'], @$this->Post['PageSize']);
        if (!empty($UserTeams)) {
    ...
    public function getDraftGameInLive_post() {
        $AuctionGames = $this->SnakeDrafts_model->getContests('ContestID,ContestGUID,AuctionStatusID,AuctionStatus,SeriesGUID,MatchGUID,GameType', array('Filter' => 'LiveAuction', 'AuctionStatusID' => 1, "LeagueType" => "Draft"), TRUE, 1);
    ...
    public function draftPlayerSold_post() {
        ...
        $Draft['Data']['MatchGUID'] = $this->Post['MatchGUID'];
    ...
    public function validateMatchStatus($UserTeamGUID) {
        $MatchStatus = $this->db->query("SELECT E.StatusID FROM nba_sports_users_teams UT, tbl_entity E WHERE UT.MatchID = E.EntityID AND UT.UserTeamGUID = '" . $UserTeamGUID . "' ")->row()->StatusID;
        if ($MatchStatus != 1) {
    ...
    public function validateUserJoinContest($ContestGUID) {
        ...
        if (!empty($ContestData)) {
            ...
            if ($ContestData['ContestType'] == 'InPlay') {
                ...
                if ($this->db->query('SELECT COUNT(*) AS `TotalRecords` FROM `nba_sports_contest_join` WHERE  `JoinInning` = "' . $this->Post['JoinInning'] . '" AND `ContestID` =' . $ContestData['ContestID'])->row()->TotalRecords >= $ContestData['ContestSize']) {
                ...
                if ($this->db->query('SELECT COUNT(*) AS `TotalRecords` FROM `nba_sports_contest_join` WHERE `ContestID` =' . $ContestData['ContestID'])->row()->TotalRecords >= $ContestData['ContestSize']) {
                    $this->form_validation->set_message('validateUserJoinContest', 'Join Contest limit is exceeded.');
                    return FALSE;
                }
                ...
                if ($ContestData['EntryType'] == 'Multiple') {
                    ...
                    /* Get User Join Limit */
                    if ($this->db->query('SELECT COUNT(*) AS `TotalJoined` FROM `nba_sports_contest_join` WHERE `ContestID` =' . $ContestData['ContestID'] . ' AND UserID = ' . $this->SessionUserID)->row()->TotalJoined >= $ContestData['UserJoinLimit']) {
    ...
    public function draftTeamSubmit_post() {
        ...
        $ContestDetails = $this->SnakeDrafts_model->getContests('RosterSize,PlayedRoster,BatchRoster,GameType,MatchID', array('ContestID' => $this->ContestID));
        ...
        $Sql = "SELECT MatchID FROM nba_sports_matches M INNER JOIN tbl_entity E ON E.EntityID=M.MatchID"
                . " WHERE E.GameSportsType = '" . $GameType . "' AND M.MatchID='" . $MatchID . "' AND M.MatchStartDateTime >='" . date('Y-m-d H:i:s', $CurrentDateTime) . "' "
                . " LIMIT 1";
        $IsMatchStart = $this->Sports_model->customQuery($Sql, TRUE);
        if (!empty($IsMatchStart)) {
            if (!$this->SnakeDrafts_model->draftTeamPlayersSubmit($this->Post, $this->UserTeamID, $this->SeriesID, $MatchID, $this->SessionUserID, $GameType)) {
                $this->Return['ResponseCode'] = 500;
                $this->Return['Message'] = "An error occurred, please try again later.";
            } else {
            ...
            $this->Return['ResponseCode'] = 500;
            $this->Return['Message'] = "Contest Started.";
        }
    }
    ...
    public function getMySquadTeam_post() {
        ...
        $this->form_validation->validation($this);  /* Run validation */
        /* Get Joined Contest Users Data */
        $AllMatchesList = $this->SnakeDrafts_model->getMySquadTeam($this->ContestID, $this->MatchID, $this->SessionUserID, $this->Post['SubGameTypeKey'], @$this->StatusID);
        if (!empty($AllMatchesList)) {
            $this->Return['Data'] = $AllMatchesList;
        }
    }
    ...
    public function removeDraftPlayer_post() {
        ...
        $this->form_validation->set_rules('MatchGUID', 'MatchGUID', 'trim');
        $this->form_validation->validation($this);  /* Run validation */

        $IsApply = $this->SnakeDrafts_model->removeDraftPlayer($this->ContestID, $this->SessionUserID, $this->UserTeamID, $this->PlayerID, $this->Post['PlayerSelectTypeRole'], $this->Post['MatchGUID']);
        if ($IsApply['Status'] == 1) {
    ...
    public function addDraftPlayer_post() {
        ...
        $IsApply = $this->SnakeDrafts_model->addDraftPlayer($this->ContestID, $this->SessionUserID, $this->UserTeamID, $this->PlayerID, $this->Post['PlayerSelectTypeRole'], $this->Post['MatchGUID']);
        if ($IsApply['Status'] == 1) {
             $this->Return['Message'] = $IsApply['Message'];
        } else {
            $this->Return['ResponseCode'] = 500;
