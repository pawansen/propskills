...
class State_model extends CI_Model
{
	...
	*/
	function addState($Input=array()){
		...
		$this->db->insert('set_location_state', $InsertData);
		return true;
	}
	...
	*/
	...
	Description: 	Use to get Cetegories
	*/
	function getCountries($multiRecords=FALSE,  $PageNo=1, $PageSize=10){
		$this->db->select('*');
        $this->db->from('set_location_country S');	
		/* Total records count only if want to get multiple records */
		if($multiRecords){ 
			$TempOBJ = clone $this->db;
			$TempQ = $TempOBJ->get();
	...
	*/
	...
	Description: 	Use to update category.
	*/
	function deleteState($Name, $CountryCode){

			/* Update User details to users table. */
			$this->db->where('StateName', $Name);
			$this->db->where('CountryCode', $CountryCode);
			$this->db->limit(1);
			$this->db->delete('set_location_state');
